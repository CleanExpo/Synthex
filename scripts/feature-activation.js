...
class FeatureActivation {
  constructor() {
    this.apiUrl = process.env.API_URL || 'https://api.synthex.social';
    this.features = {
      // Core Features
      ENABLE_AUTH: true,
  ...
  async seedInitialData() {
    ...
    try {
      ...
      await this.db.query(`
        ...
        VALUES ('admin@synthex.social', 'System Admin', 'admin', NOW())
      ...
      for (const competitor of competitors) {
        ...
        `,
          [
            competitor,
            `https://${competitor.toLowerCase().replace(' ', '')}.com`,
          ]
        );
      }
    ...
    for (const endpoint of endpoints) {
      try {
        ...
        if (response.status === endpoint.expected) {
          ...
          console.log(
            `  ⚠️ ${endpoint.path}: ${response.status} (expected ${endpoint.expected})`
          );
          allPassed = false;
        }
      } catch (error) {
