...
const SUPABASE_ANON_KEY = process.env.JWT_SECRET;
const SUPABASE_SERVICE_KEY = process.env.DATABASE_URL;
...
You can find these in your Supabase project settings.
  ...
  if (!fs.existsSync(migrationPath)) {
    console.error(
      `${colors.red}❌ Migration file not found at: ${migrationPath}${colors.reset}`
    );
    process.exit(1);
  }
  ...
  console.log(
    `${colors.green}✓${colors.reset} Migration file loaded (${migrationSQL.length} characters)`
  );
...
4. Insert sample viral patterns
  ...
  rl.question(
    `${colors.cyan}Do you want to continue? (yes/no): ${colors.reset}`,
    async answer => {
      ...
      `);
        ...
        if (!error) {
          ...
          for (const table of tables) {
            ...
            if (!tableError) {
              console.log(
                `${colors.green}✓${colors.reset} Table '${table}' exists`
              );
            } else {
              console.log(
                `${colors.yellow}⚠${colors.reset} Table '${table}' not found`
              );
            }
          }
        } else {
          console.log(
            `${colors.yellow}⚠ Could not verify database connection${colors.reset}`
          );
        }
