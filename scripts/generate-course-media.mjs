...
function validate(asset) {
  ...
  if (!ENDPOINTS[kind]) return [`unknown kind "${kind}" (expected image|video|voice)`];
  if (!r || typeof r !== 'object') return ['missing request object'];

  if (kind === 'image') {
    if (!r.prompt || r.prompt.length < 1 || r.prompt.length > 4000) e.push('image.prompt 1..4000 required');
    if (r.negativePrompt && r.negativePrompt.length > 2000) e.push('image.negativePrompt <=2000');
    if (r.aspectRatio && !IMAGE_ASPECTS.includes(r.aspectRatio)) e.push(`image.aspectRatio in ${IMAGE_ASPECTS}`);
    for (const dim of ['width', 'height']) {
      if (r[dim] != null && (r[dim] < 256 || r[dim] > 2048)) e.push(`image.${dim} 256..2048`);
    }
  }
  if (kind === 'video') {
    ...
    if (r.type === 'text-to-video' && !r.prompt) e.push('text-to-video needs prompt');
    if (r.type === 'image-to-video' && !r.imageUrl) e.push('image-to-video needs imageUrl');
    if (r.type === 'avatar' && !r.script) e.push('avatar needs script');
    if (r.duration != null && (r.duration < 1 || r.duration > 60)) e.push('video.duration 1..60');
    if (r.aspectRatio && !VIDEO_ASPECTS.includes(r.aspectRatio)) e.push(`video.aspectRatio in ${VIDEO_ASPECTS}`);
    if (r.resolution && !RESOLUTIONS.includes(r.resolution)) e.push(`video.resolution in ${RESOLUTIONS}`);
    if (r.provider && !VIDEO_PROVIDERS.includes(r.provider)) e.push(`video.provider in ${VIDEO_PROVIDERS}`);
    if (r.style && !VIDEO_STYLES.includes(r.style)) e.push(`video.style in ${VIDEO_STYLES}`);
    if (r.motionAmount && !MOTION.includes(r.motionAmount)) e.push(`video.motionAmount in ${MOTION}`);
  }
  if (kind === 'voice') {
...
function parseArgs(argv) {
  const o = { execute: false, concurrency: 3, only: null, kind: null, out: null, base: process.env.SYNTHEX_BASE_URL || null };
  const positional = [];
  for (const a of argv) {
    ...
    else if (a.startsWith('--concurrency=')) o.concurrency = Math.max(1, parseInt(a.slice(14), 10) || 3);
...
async function loadManifest(arg) {
  ...
  const p = resolve(arg);
  return { data: JSON.parse(readFileSync(p, 'utf8')), dir: dirname(p), path: p };
}
...
async function callFactory(base, token, asset) {
  ...
  if (!res.ok) {
    return { status: 'failed', httpStatus: res.status, error: body?.error || body?.message || `HTTP ${res.status}` };
  }
  // Endpoints return either a finished asset or an async job (video).
  const assetId = body?.id || body?.assetId || body?.mediaId || body?.jobId || body?.videoId || null;
  const outUrl = body?.url || body?.assetUrl || body?.output?.url || body?.result?.url || null;
  const jobStatus = body?.status || (outUrl ? 'completed' : 'submitted');
  return { status: jobStatus, assetId, url: outUrl, provider: body?.provider || asset.request.provider || null };
}
...
async function main() {
  ...
  if (!o.manifestArg) {
    console.error('Usage: node scripts/generate-course-media.mjs <manifest.json|url> [--execute] [--only=…] [--kind=image|video] [--out=…]');
    process.exit(2);
  }
  ...
  console.log(`assets selected: ${assets.length}${o.only ? ` (--only)` : ''}${o.kind ? ` (--kind=${o.kind})` : ''}`);
  console.log(`mode: ${o.execute ? 'EXECUTE (will call the media factory)' : 'DRY-RUN (validate only, no API calls)'}\n`);
  ...
  const outPath = o.out || join(dir, `${manifest.course || 'course'}-media-assets.generated.json`);
  ...
  if (!o.execute) {
    ...
    const plan = {
      ...
      willGenerate: assets.map((a) => ({ id: a.id, kind: a.kind, endpoint: ENDPOINTS[a.kind], purpose: a.purpose, placement: a.placement, tags: a.tags })),
    };
  ...
  const results = await runPool(assets, o.concurrency, async (a) => {
    try {
      ...
      const icon = r.status === 'failed' ? '✖' : r.status === 'completed' ? '✓' : '…';
      console.log(`  ${icon} ${a.id.padEnd(34)} ${r.status}${r.url ? ' → ' + r.url : ''}${r.error ? ' — ' + r.error : ''}`);
      return { id: a.id, kind: a.kind, purpose: a.purpose, placement: a.placement, tags: a.tags, ...r };
    } catch (err) {
  ...
  const map = {
    ...
    summary: {
      ...
      submitted: results.filter((r) => r.status === 'submitted' || r.status === 'processing' || r.status === 'pending').length,
      failed: results.filter((r) => r.status === 'failed').length,
    },
  };
  writeFileSync(outPath, JSON.stringify(map, null, 2));
  console.log(`\nAsset map written: ${outPath}`);
  console.log(`summary: ${map.summary.completed} completed · ${map.summary.submitted} submitted(async) · ${map.summary.failed} failed`);
  console.log('Async (video) completions land in the Synthex media library — filter by tag e.g. "ccw-workshop".');
  if (path) console.log(`Commit the asset map into CARSI beside the manifest so modules can embed finished URLs.\n`);
}

main().catch((err) => { console.error('bridge error:', err.message); process.exit(1); });
