...
for (const def of definitions.filter(d => d.required)) {
  ...
  if (!value) {
    console.log(
      `  ${colors.red}❌ ${def.name}${colors.reset} - Missing ${icon} ${def.level}`
    );
    ...
    if (!isValid) {
      console.log(
        `  ${colors.yellow}⚠️  ${def.name}${colors.reset} - Invalid format ${icon} ${def.level}`
      );
      warnings.push(`${def.name}: ${def.errorMsg || 'Invalid format'}`);
    } else {
      const maskedValue = maskValue(value, def.level);
      console.log(
        `  ${colors.green}✅ ${def.name}${colors.reset} - Set (${maskedValue}) ${icon} ${def.level}`
      );
...
for (const def of definitions.filter(d => !d.required)) {
  ...
  if (!value) {
    console.log(
      `  ${colors.gray}○ ${def.name} - Not set ${icon} ${def.level}${colors.reset}`
    );
...
for (const def of definitions) {
  ...
  if (
    (def.level === 'SECRET' || def.level === 'CRITICAL') &&
    def.name.startsWith('NEXT_PUBLIC_')
  ) {
    console.log(
      `  ${colors.red}🚨 CRITICAL: ${def.name} is ${def.level} but uses NEXT_PUBLIC_ prefix!${colors.reset}`
    );
  ...
  if (def.level === 'CRITICAL' && value.length < 32) {
    console.log(
      `  ${colors.yellow}⚠️  ${def.name} is weak (should be 32+ characters)${colors.reset}`
    );
  ...
  if (
    ...
    console.log(
      `  ${colors.yellow}⚠️  ${def.name} contains weak/common pattern${colors.reset}`
    );
...
if (securityIssues === 0) {
  ...
  console.log(
    `  ${colors.red}Found ${securityIssues} security issue(s)${colors.reset}`
  );
}
...
for (const def of definitions) {
  ...
  for (const dep of def.dependsOn) {
    if (!process.env[dep]) {
      console.log(
        `  ${colors.yellow}⚠️  ${def.name} requires ${dep} (not set)${colors.reset}`
      );
...
console.log(`${colors.cyan}\n📊 Environment Info:\n${colors.reset}`);
console.log(
  `  • NODE_ENV: ${colors.green}${process.env.NODE_ENV || 'development'}${colors.reset}`
);
console.log(`  • Platform: ${colors.green}${process.platform}${colors.reset}`);
console.log(
  `  • Node Version: ${colors.green}${process.version}${colors.reset}`
);
...
if (hasErrors) {
  ...
  console.log(
    `  1. Copy ${colors.bold}.env.example${colors.reset} to ${colors.bold}.env.local${colors.reset}`
  );
  console.log(`  2. Fill in all required variables`);
  console.log(
    `  3. Run ${colors.bold}node scripts/generate-env-docs.js${colors.reset} for documentation`
  );
  ...
  console.log(
    `  • Required: ${colors.green}${configuredRequired}/${requiredCount}${colors.reset} configured`
  );
  console.log(
    `  • Optional: ${colors.green}${configuredOptional}/${optionalCount}${colors.reset} configured`
  );
  console.log(
    `  • Total: ${colors.green}${configured.length}/${definitions.length}${colors.reset} variables set`
  );

  // Security levels breakdown
  const byLevel = {
    CRITICAL: configured.filter(v =>
      definitions.find(d => d.name === v && d.level === 'CRITICAL')
    ).length,
    SECRET: configured.filter(v =>
      definitions.find(d => d.name === v && d.level === 'SECRET')
    ).length,
    INTERNAL: configured.filter(v =>
      definitions.find(d => d.name === v && d.level === 'INTERNAL')
    ).length,
    PUBLIC: configured.filter(v =>
      definitions.find(d => d.name === v && d.level === 'PUBLIC')
    ).length,
  };

  console.log(`\n${colors.cyan}🔒 Security Levels:\n${colors.reset}`);
  console.log(
    `  ${levelIcons.CRITICAL} CRITICAL: ${byLevel.CRITICAL} configured`
  );
  console.log(`  ${levelIcons.SECRET} SECRET: ${byLevel.SECRET} configured`);
  console.log(
    `  ${levelIcons.INTERNAL} INTERNAL: ${byLevel.INTERNAL} configured`
  );
  ...
  if (missingOptional.length > 0) {
    ...
    missingOptional
      .slice(0, 5)
      .forEach(v => console.log(`  ${colors.gray}• ${v}${colors.reset}`));
    if (missingOptional.length > 5) {
      console.log(
        `  ${colors.gray}... and ${missingOptional.length - 5} more${colors.reset}`
      );
    }
  }
