...
function calculateScore(content, platform, options = {}) {
  ...
  if (config.optimalLength) {
    if (
      wordCount >= config.optimalLength.min &&
      wordCount <= config.optimalLength.max
    ) {
      score += 10;
  ...
  const hasCallToAction =
    /(click|tap|swipe|comment|share|follow|link|bio|check|visit)/i.test(
      content
    );
  ...
  switch (platform) {
    ...
    case 'linkedin':
      if (
        content.match(/\b(insight|experience|learned|professional|industry)\b/i)
      )
        score += 10;
      break;
    case 'tiktok':
...
function generateSuggestions(content, platform, score) {
  ...
  if (config.optimalLength) {
    if (wordCount < config.optimalLength.min) {
      suggestions.push(
        `Consider expanding your content (current: ${wordCount} words, optimal: ${config.optimalLength.min}-${config.optimalLength.max} words)`
      );
    } else if (wordCount > config.optimalLength.max) {
      suggestions.push(
        `Consider shortening your content (current: ${wordCount} words, optimal: ${config.optimalLength.min}-${config.optimalLength.max} words)`
      );
    }
  }
  ...
  if (platform === 'twitter' && charCount > 240) {
    suggestions.push(
      'Consider shortening to leave room for retweets and replies'
    );
  }
  ...
  if (!/#\w+/g.test(content)) {
    ...
    suggestions.push(
      `Add ${range.min}-${range.max} relevant hashtags to increase discoverability`
    );
  }
  ...
  if (
    !/\b(click|tap|swipe|comment|share|follow|link|bio|check|visit)\b/i.test(
      content
    )
  ) {
    suggestions.push(
  ...
  if (score < 60) {
    suggestions.push(
      'Content needs significant optimization for better performance'
    );
  } else if (score < 80) {
  ...
  if (suggestions.length === 0) {
    suggestions.push(
      'Excellent! Your content is well-optimized for this platform'
    );
  }
...
function getRecommendations(platform, content, score) {
  ...
  switch (platform) {
    case 'instagram':
      recs.content =
        'Use high-quality visuals and Instagram Stories for behind-the-scenes content';
      break;
    case 'facebook':
    ...
    case 'twitter':
      recs.content =
        'Create threads for longer content and use Twitter Spaces for live discussions';
      break;
    case 'linkedin':
      recs.content =
        'Share professional insights and tag relevant industry leaders';
      break;
    case 'tiktok':
      recs.content =
        'Use trending sounds and participate in challenges for maximum reach';
      break;
  }
