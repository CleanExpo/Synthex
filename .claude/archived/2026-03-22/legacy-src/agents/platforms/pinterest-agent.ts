...
export class PinterestAgent extends EventEmitter {
  constructor() {
    super();
  }
  ...
  async getContentRequirements(): Promise<any> {
    return { maxLength: 500, mediaTypes: ['image'] };
  }
  ...
  async publishPost(post: any): Promise<any> {
    return { postId: 'pinterest_' + Date.now(), success: true };
  }
}