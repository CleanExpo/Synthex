...
export default function TestSocial() {
  const [content, setContent] = useState(
    '🚀 Excited to share that our AI-powered marketing platform is transforming how businesses connect with their audience!\n\nKey features:\n✅ Smart content generation\n✅ Multi-platform posting\n✅ Real-time analytics\n✅ Viral pattern detection'
  );
  ...
  const [hashtags, setHashtags] = useState(
    '#AI #Marketing #SocialMedia #Innovation'
  );
  ...
  const postToSocial = async () => {
    ...
    try {
      ...
      if (data.success) {
        toast.success(
          `Successfully posted to ${data.results.length} platform(s)!`
        );
        // Add to history
        setPostHistory(prev => [
          {
  ...
  const getCharacterCount = () => {
    const minLength = Math.min(
      ...selectedPlatforms.map(
        p => PLATFORMS.find(plat => plat.id === p)?.maxLength || Infinity
      )
    );
    return {
      ...
      percentage:
        minLength === Infinity ? 0 : (content.length / minLength) * 100,
    };
  };
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <div className="flex justify-between mt-2 text-sm">
                    <span
                      className={`${charCount.percentage > 90 ? 'text-red-500' : 'text-gray-500'}`}
                    >
                ...
                <Button
                  ...
                  size="lg"
                  disabled={
                    loading || !content || selectedPlatforms.length === 0
                  }
                >
                  {loading ? (
                    <>
                ...
                <CardContent>
                  ...
                  {results.results && results.results.length > 0 && (
                    <div className="mt-4 space-y-2">
                      {results.results.map(
                        (
                          result: { platform: string; success: boolean },
                          i: number
                        ) => (
                          <div
                            key={i}
                          ...
                          >
                            <div className="flex items-center space-x-2">
                              {PLATFORMS.find(p => p.id === result.platform)
                                ?.icon && (
                                <div
                                  className={`p-1 rounded ${PLATFORMS.find(p => p.id === result.platform)?.color}`}
                                >
                                  {(() => {
                  ...
                  {results.errors && results.errors.length > 0 && (
                    <div className="mt-4 space-y-2">
                      ...
                      {results.errors.map(
                        (
                          error: { platform: string; error: string },
                          i: number
                        ) => (
