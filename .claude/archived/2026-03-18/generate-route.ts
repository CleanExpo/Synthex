...
const enhancedRateLimiters = {
  aiGeneration: (req: NextRequest, handler: () => Promise<NextResponse>) =>
    aiGeneration(req, handler),
  ai: (req: NextRequest, handler: () => Promise<NextResponse>) =>
    aiGeneration(req, handler),
};
      ...
      return NextResponse.json(
        {
          ...
          message:
            'Content generation requires an AI API key. Contact support to enable this feature.',
        },
        { status: 501 }
      );
    }
      ...
      return NextResponse.json(
        {
          ...
          message: 'Content generation failed. Please try again later.',
        },
        { status: 502 }
      );
    }
