# Hook craft — first-line surfaces (LinkedIn · ad copy · thread openers · thought-leadership ledes)

Loaded on demand by `senior-copywriter` when a draft's opening line carries the click.
The conversion hypothesis (M-2) measures the hook; this reference is how the hook is built.

## A hook is a contract

A hook makes a **promise** and opens a **curiosity gap** the body must pay off. The reader
advances only because the first line committed to a specific payoff and withheld just enough
to make stopping feel like a loss.

A hook is **not**: a summary, a greeting, a title, or clever wordplay. If the first line
could be the H1 of the page, it is a title, not a hook — rewrite it.
...
1. **Generate 30.** Constrain the batch across `archetype × angle × length` — not 30 variations
   of one idea. Force coverage of at least 5 archetypes.
2. **Kill to 10.** The best hooks are usually **#17–23**, not #1–6 — the obvious openings
   surface first and are the most generic. Do not stop early.
3. **A/B the 3.** Ship the top 3 as measurable variants; the conversion hypothesis names the
   metric, window, and kill threshold (per M-2).
...
## The 6 hook-killers (mechanical reject — with the rewrite move)
