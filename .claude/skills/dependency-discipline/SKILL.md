...
| Media processing (ffmpeg)   | Railway media worker + `media_*` MCP tools + `scripts/` frame extraction — corpus-growth path from owned videos into the reference library                                                                                                                                                                                                  |
...
- ❌ Direct image/video provider calls (Gemini/OpenAI/Stability/fal) or stock-photo APIs — the
  static guard test `tests/unit/ai/no-direct-image-apis.test.ts` fails CI on them;
