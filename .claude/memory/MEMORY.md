...
[22/05/2026] DECISION (SYN-968, M12 Slice 3): Persist governed Marketing Agency signals, opportunities, and outcome events in organization-scoped Prisma tables before UI exposure | REASON: Slice 1/2 proved the service contract and Apify adapter; Command Centre and Health Loop work now need durable ontology records that preserve source evidence, signal score, risk state, approval gate, and outcome-learning hooks across agent runs. The Apify command persists only when `MARKETING_AGENCY_SIGNAL_ORGANIZATION_ID` is configured, keeping provider execution JSON-only by default. | ALTERNATIVES REJECTED: UI-first display of transient JSON (no durable learning loop), direct provider-shaped database rows (policy leaks into adapters), unconditional Apify persistence (would write without an explicit org boundary)
