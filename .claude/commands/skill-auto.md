...
| Image/Video | image, video, visual, photo, thumbnail, reel, generate image, generate video | grounded-visuals (MANDATORY), visual-content-brief, video-engine |

> **Image/Video is non-negotiable**: any image or video generation MUST go through
> `generateImage()`/`generateBatch()` in `lib/services/ai/image-generation.ts` (or the
> `generate_image`/`generate_video` MCP tools), grounded on
> `public/reference-library/manifest.json` with trained LoRAs (carpet-style-v1 /
> `ccwcarpet`) auto-applied. No owned references ⇒ BLOCKED. See
...
**Exception — image/video generation:** never generate a new ad-hoc image-generation
skill. `grounded-visuals` already exists and is mandatory for every visual asset
request (see the Image/Video row above and `.claude/rules/real-images-only.md`).
...
| "create an image for the CCW carpet post" | grounded-visuals, visual-content-brief      |
