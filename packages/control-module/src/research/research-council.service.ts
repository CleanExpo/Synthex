  ...
  return [
    {
      ...
      claim:
        'Treat the request as a question-led research brief before creative production.',
    ...
    },
    {
      ...
      evidenceRefs: evidenceOrPlaceholder(evidenceRefs),
      confidence: referencesTechnicalWork(input.cleanedText)
        ? 0.82
        : baseConfidence,
    ...
    },
    {
      ...
      evidenceRefs: evidenceOrPlaceholder(evidenceRefs),
      confidence: referencesCreativeWork(input.cleanedText)
        ? 0.84
        : baseConfidence,
    ...
    },
    {
      ...
      risks: hasEvidence
        ? []
        : ['Research council must block production without source refs.'],
      nextAction:
...
function buildOpenQuestions(
  ...
  findings
    ...
    .forEach(risk => questions.add(risk));

  if (
    !/\bmetric|roi|views|clicks|lead|conversion|rank\b/i.test(input.cleanedText)
  ) {
