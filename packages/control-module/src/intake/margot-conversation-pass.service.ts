...
export function runMargotConversationPass(
  ...
  const risks = RISK_PATTERNS.filter(([pattern]) =>
...
function classifySensitivity(text: string, risks: string[]): Sensitivity {
  ...
  if (/\b(client|board|internal|margin|pricing)\b/i.test(text))
    return 'internal';
  return 'public';
}

function classifyIntent(text: string): MargotConversationPassResult['intent'] {
