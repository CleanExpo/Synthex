...
export function buildHermesHandoffReadiness(
  ...
  if (!runtime.telegramConfigured) {
    missing.push(
      'Telegram channel (TELEGRAM_BOT_TOKEN + TELEGRAM_CHAT_ID) not fully configured'
    );
  }
  if (!runtime.whatsappConfigured) {
    missing.push(
      'WhatsApp channel (WHATSAPP_ACCESS_TOKEN + WHATSAPP_PHONE_NUMBER_ID) not configured'
    );
  }
...
function buildHermesSourceMap(
  ...
  return [
    ...
    },
    {
      ...
      route: '/api/command-centre/intake',
      guardrail:
        'Blocked until WhatsApp is configured and contact policy passes.',
    },
    {
      channel: 'plaud',
