...
describe('GET /api/workflows/executions', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    (subscriptionService.getSubscription as jest.Mock).mockResolvedValue(
      mockProfessionalSubscription
    );
  });
...
describe('POST /api/workflows/executions', () => {
  ...
  it('creates content-campaign execution without explicit steps (AT-028)', async () => {
    (APISecurityChecker.check as jest.Mock).mockResolvedValue(
      mockSecurityAllowed
    );
    (prisma.user.findUnique as jest.Mock).mockResolvedValue(mockUser);
    ...
    const req = createMockRequest({
      method: 'POST',
      body: { title: 'Campaign', template: 'content-campaign' },
    });
    const res = await POST(req);
    expect(res.status).toBe(201);
    expect(prisma.workflowExecution.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          totalSteps: 8,
          inputData: expect.objectContaining({ agencyTaskId: 'AT-028' }),
        }),
      })
    );
  });
