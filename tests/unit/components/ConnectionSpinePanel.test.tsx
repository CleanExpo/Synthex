 ...
 *
 * Render completion is driven by `settle()` (tests/helpers/settle.ts), not by a
 * wall-clock `waitFor` window — see that file for why.
 */
...
describe('ConnectionSpinePanel (SYN-1030)', () => {
  ...
  it('renders an error state when the request fails', async () => {
    ...
    await settle();
    expect(
      screen.getByText(/couldn't load connection health/i)
    ).toBeInTheDocument();
  });
        ...
        systems: [
          {
            ...
            action:
              'Connect the Obsidian / 2nd-brain vault to enable writeback.',
          },
        ],
      })
    );
