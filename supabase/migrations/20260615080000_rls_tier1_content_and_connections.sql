...
-- Safety:    Table AND column existence guards — safe against preview rebuild gaps
--            where organization_id may not yet be present (added outside migrations).
-- ============================================================================
...
--    Guard: organization_id column may not exist in preview — fall back to user_id only.
-- ============================================================================
DO $campaigns$
DECLARE
  has_org_col boolean;
...
IF to_regclass('public.campaigns') IS NOT NULL THEN
  ...
  IF has_org_col THEN
    CREATE POLICY campaigns_select ON public.campaigns FOR SELECT TO authenticated
      USING (
        ...
        OR (organization_id IS NULL AND user_id = (SELECT auth.uid())::text)
      );
  ...
  ELSE
    -- Preview fallback: scope to user_id only (organization_id column absent)
    CREATE POLICY campaigns_select ON public.campaigns FOR SELECT TO authenticated
...
-- ============================================================================
-- 2. POSTS (inherit from parent campaign)
-- ============================================================================
DO $posts$
BEGIN
IF to_regclass('public.posts') IS NOT NULL THEN
  ...
  -- Posts inherit org scope through their parent campaign.
  -- The subquery is safe even if campaigns lacks organization_id — the
  -- campaigns_select policy already handles that case and posts can only
  -- be reached through a campaign the user can see.
...
-- ============================================================================
-- 3. PLATFORM_CONNECTIONS (OAuth tokens — highest sensitivity)
--    Guard: organization_id column may not exist in preview.
-- ============================================================================
...
-- ============================================================================
-- 4. PLATFORM_POSTS (inherit from platform_connection)
-- ============================================================================
DO $platform_posts$
BEGIN
  ...
  CREATE POLICY platform_posts_select ON public.platform_posts FOR SELECT TO authenticated
    USING (EXISTS (
      SELECT 1 FROM public.platform_connections pc WHERE pc.id = connection_id
        AND pc.user_id = (SELECT auth.uid())::text
    ));
...
-- ============================================================================
-- 5. PLATFORM_METRICS (inherit through platform_post → platform_connection)
-- ============================================================================
DO $platform_metrics$
BEGIN
IF to_regclass('public.platform_metrics') IS NOT NULL THEN
  ...
  CREATE POLICY platform_metrics_select ON public.platform_metrics FOR SELECT TO authenticated
    USING (EXISTS (
      ...
      WHERE pp.id = post_id AND pc.user_id = (SELECT auth.uid())::text
    ));
