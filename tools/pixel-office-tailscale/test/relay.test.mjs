...
test('tagCwd appends the machine to a POSIX leaf and keeps the separators', () => {
  assert.equal(tagCwd('/Users/phill-mac/proj', 'unite-mac-mini'), '/Users/phill-mac/proj@unite-mac-mini');
});

test('tagCwd keeps Windows backslashes intact', () => {
  assert.equal(tagCwd(winPath, 'phill-desktop'), `D:${BACKSLASH}Synthex${BACKSLASH}apps${BACKSLASH}web@phill-desktop`);
});

test('tagCwd tolerates a trailing separator', () => {
  assert.equal(tagCwd('/Users/phill-mac/proj/', 'mac'), '/Users/phill-mac/proj@mac');
});
  ...
  const out = tagEvent(
    {
      ...
      session_id: 'abc-123',
      transcript_path: '/Users/phill-mac/.claude/projects/-Users-phill-mac-proj/abc-123.jsonl',
...
test('tagEvent does not mutate the caller\u2019s event', () => {
  const original = { hook_event_name: 'Stop', transcript_path: '/x.jsonl', cwd: '/a/b' };
...
test('tagEvent supplies a cwd when the event has none', () => {
  const out = tagEvent({ hook_event_name: 'SessionStart', session_id: 's' }, 'mac');
