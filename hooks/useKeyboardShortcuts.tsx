...
export function useKeyboardShortcuts() {
  ...
  // Define action callbacks first so the shortcuts array can reference them stably
  ...
  // Memoised so the array reference is stable between renders — prevents the keydown
  // listener from being re-registered on every render cycle.
  const shortcuts: Shortcut[] = useMemo(
    () => [
      ...
      // Actions
      { key: 's', ctrl: true, action: saveCurrentWork, description: 'Save' },
      { key: 'z', ctrl: true, action: undo, description: 'Undo' },
      { key: 'z', ctrl: true, shift: true, action: redo, description: 'Redo' },
      { key: 'Escape', action: closeModals, description: 'Close modal/menu' },
      {
        key: '?',
        shift: true,
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-gray-500 mt-4">
        Pro tip: Press{' '}
        <kbd className="px-1 py-0.5 text-xs bg-gray-800 rounded">?</kbd> anytime
        to see shortcuts
      </p>
    </div>
  );
