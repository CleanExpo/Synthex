...
| 7   | **Scheduled report delivery**                                                                     | PRESENT `[VERIFIED]` | v1       | working cron `app/api/reports/scheduled/execute/route.ts` (Resend+SendGrid send, updates `ScheduledReport`, records `ReportDelivery`); model `schema.prisma:6478`; CRUD `app/api/reports/scheduled/route.ts`; dashboard `app/dashboard/reports` | shipped #1/#467: `sendReportEmail` attaches real file via `buildReportAttachments` (`format:'pdf'`→jsPDF); dead `ScheduledReportManager` removed     |
| 8   | **Attribution (channel-agnostic, paid-ready)**                                                    | PRESENT `[VERIFIED]` | v1\*     | `lib/analytics/analytics-tracker.ts` (UTM), `app/api/effect-report/route.ts`, `lib/attribution/compute` (first/last/linear/time-decay)                                                                                                          | shipped #7/e3ab91fa: multi-touch models + reserved paid ad shape + empty paid-report slot seam                                                       |
| 9   | **Sentiment & social listening**                                                                  | PRESENT `[VERIFIED]` | v1       | `lib/social/sentiment-analyzer.ts:89` (`getAIProvider()`), `app/api/analytics/sentiment/route.ts`, `app/api/listening/*`                                                                                                                        | shipped: surfaced in dashboard #11/#475                                                                                                              |
| 10  | **Brand & voice** (DNA, voice scoring, consistency, competitive-intel + content gap)              | PRESENT `[VERIFIED]` | v1       | `lib/brand-dna/extractor.ts`, `lib/brand-voice/quality-scorer.ts`, `lib/services/competitive-intel.ts` (1228 lines, `:575 identifyContentGaps`)                                                                                                 | shipped: persona training loop #8/#477; competitive-intel UI #10/#474+#478                                                                           |
| 11  | **Strategy / planning / campaigns**                                                               | PARTIAL `[VERIFIED]` | v1       | real CRUD `app/api/campaigns/route.ts:262` (`tx.campaign.create`); `full-campaign-generator.ts:506` **unwired** (scripts/tests only); demo route `app/api/marketing-agency/campaigns/route.ts:11` mock                                          | shipped: authority generator wired #2/#469 (real persisted route)                                                                                    |
...
| 13  | **Email marketing / lifecycle**                                                                   | PRESENT `[VERIFIED]` | v1       | `lib/email/` (18 modules: `queue.ts`, `email-service.ts`, `effect-report-email.ts`, milestone/monthly/quarterly)                                                                                                                                | v1 scope closed #13/#549; email-as-channel + newsletter UI deferred to v2                                                                            |
...
| 15  | **Approvals & collaboration**                                                                     | PARTIAL `[VERIFIED]` | v1       | `app/api/approvals/route.ts`, `prisma` `ApprovalRequest`, `app/dashboard/{approvals,collaboration}/`                                                                                                                                            | shipped: threaded comments + @-mentions + SLA cron #12/#483+#542                                                                                     |
| 16  | **Billing & commercials** (Stripe SaaS, usage, dunning, invoices+PDF, GST)                        | PARTIAL `[VERIFIED]` | v1       | `lib/stripe/*`, `app/api/invoices/route.ts`, `app/api/invoices/[id]/pdf/route.ts` (puppeteer)                                                                                                                                                   | shipped: agency invoicing #5/#550; promo-code discounts #14 (`lib/stripe/promo-code.ts`)                                                             |
| 17  | **Client / CRM**                                                                                  | PARTIAL `[VERIFIED]` | v1       | primitives: `prisma` `Lead:6446`, `DealDeliverable:1661`, `ClientHealthScore:6155`, `ClientEngagementEvent:6214`, `PipelineCostLedger:5751`; org-switch `app/api/businesses/switch/route.ts`                                                    | shipped: CRM rollup + console #3/#545-#548                                                                                                           |
| 18  | **Multi-client console (in-house)**                                                               | MISSING `[VERIFIED]` | v1       | org-switch + `parentOrgId` traversal exist (infra only)                                                                                                                                                                                         | shipped: console at `app/dashboard/clients` #4/#548                                                                                                  |
| 19  | **Influencer / UGC management**                                                                   | MISSING `[VERIFIED]` | v1       | grep = no real domain                                                                                                                                                                                                                           | API+models shipped #6 (`Creator`/`UgcSubmission`, `app/api/{ugc,ugc/promote,creators}`); **remaining v1 gap: dashboard UI** _(founder §14-1: in v1)_ |
...
| 3   | ✅ **SHIPPED** (clients console slice 1 `#471`; `#532`: `Contact` model + `clientId`/`crmClientId` columns; migration `20260618_add_contacts_and_crm_links` (contacts table + RLS + 4 client-link columns); `#540`: Zod `/api/contacts` list+create route + tests; `#545`: `/api/contacts/[id]` GET/PATCH/DELETE org-scoped detail CRUD, 13 tests; `#546`: `GET /api/clients/[clientId]/crm` — read-only unified rollup of org-scoped `Contact`/`Lead`/`ClientHealthScore`/`ClientEngagementEvent` for one soft client id, 6 tests; `#547`: folds `DealDeliverable` into the same rollup, org-safe via the `deal.sponsor.user.organizationId` relation chain (no own `organizationId`) + soft `clientId`, 7 tests; this PR: clients-console detail view now surfaces the rollup (`ClientCrmPanel` fetches `GET /api/clients/[id]/crm` for the selected client — health badge + contacts/leads/deliverables/engagement sections). **CRM unification complete** — unified client list + detail from existing tables, org-scoped CRUD + Zod throughout) **Unify CRM primitives** — fold `Lead`/`DealDeliverable`/`ClientHealthScore`/`ClientEngagementEvent` into `Client`/`Contact` + client console | CRM (#17)                 | L   | High     | P2/P3/P1 | `code-architect`+`senior-reviewer`        | Unified client list/detail from existing tables; org-scoped CRUD + Zod                                                                                                                                                                                                                                                        |
...
| 5   | ✅ **SHIPPED** (`UNI-173` + DELETE paid-guard/`/list` `#541`) **Agency / client-facing invoicing** — invoice an org's end-clients via Stripe; margin/passthrough                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Billing (#16)             | L   | High     | P2/P3    | `code-architect`                          | ✅ DONE (UNI-173): client invoice w/ line items + 10% GST, status workflow (draft→sent→paid→overdue→cancelled), Stripe payment-link, Resend send, PDF; `/api/invoices` (create), `/[id]` (GET/PATCH/DELETE), `/list`. Tests: invoices.test.ts (POST/GET/PATCH) + #541 (DELETE paid-guard + /list org-scope/filter/pagination) |
| 6   | ✅ **SHIPPED** (`#531`: `Creator`+`UgcSubmission` models + `/api/creators` with Zod + RLS migration; `#536`: creators route tests; `#537`: `/api/ugc` GET+POST intake + PATCH moderate + `promoteApprovedSubmission` worker; `#544`: `POST /api/ugc/promote` trigger — approved submission → ContentCalendar slot via SYN-1040 `persistPublishSlot`, org-scoped, typed-error→HTTP mapping, 11 route tests) **Influencer / UGC management** — creator records + outreach + UGC intake workflow                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Influencer (#19)          | L   | Med-High | P2/P3/P1 | `code-architect`                          | Creator CRM + UGC intake; org-scoped; intake feeds the content pipeline                                                                                                                                                                                                                                                       |
...
| 12  | ✅ **SHIPPED** (`#483` + RLS reconcile `#485` + SLA reminders `#542`) **Approvals collaboration** — threaded comments API, @-mentions, overdue-approval SLA reminders                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Approvals (#15)           | M   | Med      | P3/P1    | `code-architect`                          | DONE: threaded comments persisted + @-mentions; org-member RLS aligned; daily SLA cron notifies the submitter of lapsed-due-date approvals (dedupe-stamped). 5 worker tests green                                                                                                                                             |
| 13  | ✅ **SHIPPED (v1 scope)** (Twitter parity `#476` + IG Reels backend `#480` + Reel-slot media API `#543` w/ 9 org-scoped tests; composer media-uploader UI confirmed live — `MediaAttacher` wired to `POST /api/media/upload`, mounted in `app/dashboard/content/page.tsx`. **Deferred to v2:** email-as-channel — speculative, needs NEW infra (no recipient model exists, only the `EmailCampaign` tracker; requires an `EmailSubscriber`/recipient model + email-provider decision + a new `publishQueue` adapter case), so out of v1 per use-existing-infra) **Publishing polish** — IG Stories/Reels UI; email-as-channel; Twitter `createPost` parity                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Publishing (#2,#13)       | M   | Low      | P1/P3    | `code-architect`                          | Story/Reel scheduling UI; email channel in queue; Twitter conforms to base                                                                                                                                                                                                                                                    |
...
data when a quota is exhausted** (the PageSpeed demo fallback was removed in #9/#472; the
service now states unavailability instead).
...
| `[VERIFIED]` resolved | **Scheduled-report delivery + PDF attachment shipped** (cron sends via Resend/SendGrid, updates `ScheduledReport`, records `ReportDelivery`) — `sendReportEmail` now attaches the real file via `buildReportAttachments` (`format:'pdf'` → jsPDF), dead `ScheduledReportManager` removed | Shipped in #1/#467 (`lib/reports/report-attachments.ts`, test `tests/unit/reports/report-attachments.test.ts`). Every other backlog item gets the same verify-before-build pass via its child spec |
...
| `[VERIFIED]` resolved | **PageSpeed demo-score fabrication removed** — `pagespeed-service.ts` now returns null scores + states unavailability when the PSI key/API is missing                                                                                                                                    | Shipped in #9/#472; no fabricated 85/95 remain                                                                                                                                                     |
