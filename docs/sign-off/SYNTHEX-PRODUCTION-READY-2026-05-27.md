...
|---|---|
| Date | 2026-05-27 |
| Evidence captured | 2026-05-27 01:09-04:39 UTC |
...
| RLS schema coverage command | BLOCKED local source | `npm run rls:coverage` now runs through plain Node instead of `tsx`; it reaches the real validator and currently fails with 62 Prisma models lacking matching `ALTER TABLE ... ENABLE ROW LEVEL SECURITY` coverage in migrations. This is a source migration coverage blocker, separate from the live Supabase adversarial check. |
...
| RLS schema coverage | BLOCKED LOCAL SOURCE | `npm run rls:coverage` fails with 62 uncovered Prisma models. Either add forward migrations that enable RLS and appropriate policies, or intentionally exempt tables with documented justification before `/shipit`. |
...
Synthex is now consolidated around the main GitHub repo and local source validation is green at source gate baseline commit `c319db75`, with follow-up readiness evidence, production smoke script repair, cron route hardening, and readiness Redis probe alignment layered on top.
