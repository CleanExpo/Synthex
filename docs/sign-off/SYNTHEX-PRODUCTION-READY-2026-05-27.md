...
|---|---|
| Date | 2026-05-27 |
| Evidence captured | 2026-05-27 01:09-04:44 UTC |
...
| RLS schema coverage command | BLOCKED local source | `npm run rls:coverage` now runs through plain Node instead of `tsx`; it reaches the real validator and currently fails with 43 Prisma models lacking matching RLS coverage. This is a source migration coverage blocker, separate from the live Supabase adversarial check. |
...
| RLS schema coverage | BLOCKED LOCAL SOURCE | `npm run rls:coverage` now fails with 43 uncovered Prisma models after a narrow policy-backed enablement migration reduced the previous 62 uncovered models. Either add forward migrations that enable RLS and appropriate policies, or intentionally exempt tables with documented justification before `/shipit`. |
