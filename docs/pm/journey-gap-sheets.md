...
| AT-010 | Brief → creative-director → REM asset             | **Product:** pending Remotion brief ships via `POST /api/admin/remotion`; server/Lambda MP4 render + REM protocol in workflow steps still open               | Eng     | M      |
...
| AT-015 | DR GBP / schema                                   | **Product:** `/dashboard/google-business` COMPLETE — OAuth connect + org-matched location sync; honest empty until connected/synced (no fabricated listings) | Eng     | —      |
