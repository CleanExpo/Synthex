...
class AdminDashboard {
  ...
  initEventListeners() {
    ...
    document
      .getElementById('user-plan-filter')
  ...
  async loadSectionData(section) {
    try {
      ...
      this.showAlert(
        'error',
        `Failed to load ${section} data: ` + error.message
      );
    }
  }
  ...
  async acknowledgeAlert(alertId) {
    try {
      await this.apiRequest(`/alerts/${alertId}/acknowledge`, {
        method: 'POST',
      });
  ...
  getAlertClass(level) {
    switch (level) {
      case 'critical':
        return 'alert-critical text-white';
      case 'warning':
        return 'alert-warning text-white';
      case 'info':
