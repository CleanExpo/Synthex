...
async function initializeRedisService() {
  ...
  if (!redisService && isVercel && hasRedisCloud && !isEdgeRuntime) {
    try {
      const { default: redisCloudVercel } =
        await import('./redis-cloud-vercel.js');
  ...
  if (!redisService) {
    console.warn(
      '⚠️ No Redis configuration found - using memory-only fallback'
    );
...
function createClusterServiceWrapper(client) {
  return {
    ...
    async createSession(userId, sessionData, ttl = 86400) {
      ...
      await this.set(
        `session:${sessionId}`,
        { id: sessionId, userId, ...sessionData },
        ttl
      );
      return sessionId;
    },
    ...
    async healthCheck() {
      ...
      const healthy = Boolean(health.connected);
      return {
        healthy,
...
export const createSession = (...args) =>
  unifiedRedisService.createSession(...args);
export const getSession = (...args) => unifiedRedisService.getSession(...args);
export const updateSession = (...args) =>
  unifiedRedisService.updateSession(...args);
export const deleteSession = (...args) =>
  unifiedRedisService.deleteSession(...args);
export const getUserSessions = (...args) =>
  unifiedRedisService.getUserSessions(...args);
export const checkRateLimit = (...args) =>
  unifiedRedisService.checkRateLimit(...args);
export const healthCheck = (...args) =>
  unifiedRedisService.healthCheck(...args);
