...
export async function execute(stepDef: WorkflowStepDefinition, context: StepContext): Promise<StepResult> {
  try {
    ...
    const thinkingEffort = stepDef.config?.thinkingEffort as string | undefined
    ...
    const result = await provider.complete({
      ...
      thinkingDisplay: 'visible', // Default to showing thinking
    ...
    return {
      success: true,
      output: { content: content, metadata: { model: result.model ?? 'unknown', tokensUsed } },
