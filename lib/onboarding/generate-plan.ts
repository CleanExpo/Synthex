/**
 * Onboarding — 90-Day Marketing Plan Service (SYN-23)
 *
 * Turns the 6-question onboarding answers into a personalised 90-day marketing
 * plan using the premium AI model (quality > speed here). Ported from the
 * archived route logic (.claude/archived/2026-03-13/onboarding-api/generate-plan)
 * into lib/ so the API route stays thin (app/api → lib → Prisma).
 *
 * Pure service: no auth, no DB. Given validated answers, returns a MarketingPlan
 * (or FALLBACK_PLAN if the AI call fails or returns malformed JSON).
 */
...
export const FALLBACK_PLAN: MarketingPlan = {
...
export async function generatePlanWithAI(answers: GoalsAnswers): Promise<MarketingPlan> {
  try {
