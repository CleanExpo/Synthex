 ...
 * config/platform-master-config.json.
 *
...
import platformConfig from '@/config/platform-master-config.json';
...
function buildSystemPrompt(
  ...
  const optimalTime =
    config.posting_schedule?.optimal_times?.[0] ?? 'check platform analytics';
...
export class PlatformSpecialistAgent {
  ...
  async enhance(params: EnhanceParams): Promise<EnhancedPost> {
    const { content, platform, businessName, businessIndustry, tone, orgId } =
      params;

    // Circuit breaker guard
    if (!checkCircuitBreaker(orgId)) {
      logger.warn('platform-specialist: circuit breaker tripped', {
        orgId,
        platform,
      });
...
Apply the ${config.winning_formulas[0].template} formula. Return ONLY the JSON response.`;
    ...
    try {
      ...
      const confidenceScore = Math.min(
        1.0,
        Math.max(0.0, parsed.confidenceScore ?? 0.5)
      );
      ...
      return {
        ...
        metadata: {
          ...
          characterCount: enhancedContent.length,
          optimalPostingTime:
            parsed.optimalPostingTime ??
            config.posting_schedule?.optimal_times?.[0] ??
            '',
          winningTemplate:
            parsed.winningTemplate ?? config.winning_formulas[0].template,
          confidenceScore,
        },
        suggestions: (parsed.suggestions ?? []).slice(0, 3),
      };
    } catch (err) {
      logger.error('platform-specialist: enhancement failed', {
        orgId,
        platform,
        error: err,
      });
