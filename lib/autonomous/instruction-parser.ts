...
const parsedOutputSchema = z.object({
  ...
  intents: z.array(z.enum([
    'create', 'analyse', 'enrich', 'schedule',
    'publish', 'notify', 'review', 'research', 'optimise',
  ])),
})
...
export async function parseInstruction(
  ...
  try {
    ...
    throw new Error('Failed to parse AI response — the instruction may be too complex')
  }
  ...
  if (!validated.success) {
    ...
    throw new Error('AI response did not match expected format — try rephrasing your instruction')
  }
