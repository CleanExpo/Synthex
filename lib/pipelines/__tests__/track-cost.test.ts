...
describe('calculateCostUsd', () => {
  it('claude-opus-4-6: $15 input + $75 output per 1M tokens', () => {
    expect(
      calculateCostUsd('claude-opus-4-6', 1_000_000, 1_000_000)
    ).toBeCloseTo(90, 4);
  });
  ...
  it('claude-haiku-4-5: $0.25 input + $1.25 output per 1M tokens', () => {
    expect(
      calculateCostUsd('claude-haiku-4-5', 1_000_000, 1_000_000)
    ).toBeCloseTo(1.5, 4);
  });
  ...
  it('handles small token counts with 6-decimal precision', () => {
    ...
    expect(calculateCostUsd('claude-haiku-4-5', 1000, 2000)).toBeCloseTo(
      expected,
      6
    );
  });
