...
export class ContentGeneratorService {
  ...
  async generateContent(params: {
    ...
    try {
      ...
      const requirements =
        PLATFORM_REQUIREMENTS[platform as keyof typeof PLATFORM_REQUIREMENTS];
      ...
      const optimizedContent = this.optimizeForPlatform(
        voicedContent,
        platform,
        requirements
      );
      ...
      const finalContent = this.enhanceContent(optimizedContent, {
        includeHashtags,
        includeEmojis,
        platform,
      });
  ...
  private generateContentStructure(hookType: string, topic: string): string {
    const templates =
      CONTENT_TEMPLATES[hookType as keyof typeof CONTENT_TEMPLATES] ||
      CONTENT_TEMPLATES.question;

    // Select templates deterministically based on topic + hookType
    const seed = `${hookType}:${topic}`;
    const starter = this.selectDeterministic(
      templates.starters,
      seed + ':start'
    );
...
Maintain the core message but adapt the voice to be ${persona.attributes.emotion}.`;
  ...
  private replacePlaceholders(content: string, data: PlaceholderData): string {
    ...
    result = result.replace(
      /{number}/g,
      String((this.djb2(topicSeed) % 5) + 3)
    );
    result = result.replace(
      /{adjective}/g,
      this.selectDeterministic(
        ['amazing', 'incredible', 'unexpected', 'surprising'],
        topicSeed + 'adj'
      )
    );
    result = result.replace(
  ...
  private addHashtags(content: string, platform: string): string {
    ...
    const platformTags =
      hashtags[platform as keyof typeof hashtags] || hashtags.twitter;
  ...
  private predictEngagement(content: string, platform: string): number {
    ...
    const optimal =
      optimalLength[platform as keyof typeof optimalLength] || 100;
