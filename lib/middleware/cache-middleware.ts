 ...
 *
 * Migrated from src/middleware/cache-middleware.ts to use lib/cache/cache-manager
 * instead of src/infrastructure/ dependencies.
 */
...
export async function invalidateCache(pattern: string): Promise<number> {
  try {
    const cache = getCache();
    const deleted = await cache.invalidateByTag(`api:${pattern}`);
...
export async function clearApiCache(): Promise<void> {
  try {
    const cache = getCache();
    await cache.invalidateByTag('api:');
