...
The skill docs now *say* "score via the real scorer / route through
...
*DoD:* `grep -rn "Math.random" app/api/ai/` returns nothing for scores; a new
...
*DoD:* response includes a real `brandVoiceScore` when a profile exists, omitted
...
*DoD:* threshold configurable, tests for 401 → 403 → below-threshold → 200; explicit
...
- `QualityScorer` may call AI → kept advisory + behind the existing
  graceful-disable so there's no new *mandatory* spend; org-scoped via the
