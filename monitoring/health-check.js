...
class HealthMonitor {
  constructor(config) {
    this.config = {
      apiUrl: process.env.API_URL || 'https://api.synthex.social',
  ...
  async checkService(service) {
    ...
    try {
      ...
      this.failureCount[service.name] =
        (this.failureCount[service.name] || 0) + 1;
  ...
  getMetrics() {
    ...
    const uptimePercentage = (
      ((uptime.checks - uptime.failures) / uptime.checks) *
      100
    ).toFixed(2);

    // Calculate average response times
    const avgResponseTimes = {};
    for (const [service, times] of Object.entries(
  ...
  logResults(results) {
    ...
    console.log(
      `[${timestamp}] Health Check: ${healthy} healthy, ${unhealthy} unhealthy`
    );
