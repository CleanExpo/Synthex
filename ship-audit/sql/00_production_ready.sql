-- =================================================================
-- PRODUCTION-READY DATABASE MIGRATION FOR SUPABASE
-- Generated: 2025-08-13
-- 
-- This version checks table existence before ALL operations
-- =================================================================
...
BEGIN
    ...
    IF table_exists('platform_connections') AND table_exists('users') THEN
    ...
    IF table_exists('platform_posts') AND table_exists('platform_connections') THEN
        v_column := get_column_name('platform_posts', ARRAY['connectionId', 'connection_id']);
        IF v_column IS NOT NULL AND NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_platform_posts_connection') THEN
    ...
    IF table_exists('platform_metrics') AND table_exists('platform_posts') THEN
        v_column := get_column_name('platform_metrics', ARRAY['postId', 'post_id']);
        IF v_column IS NOT NULL AND NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_platform_metrics_post') THEN
...
BEGIN
    ...
    IF table_exists('api_usage') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'api_usage' AND column_name = 'status')
    ...
    IF table_exists('organizations') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'organizations' AND column_name = 'plan')
...
BEGIN
    ...
    IF table_exists('posts') THEN
        ...
        IF v_scheduled_at_col IS NOT NULL AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'posts' AND column_name = 'status') THEN
            ...
            RAISE NOTICE 'Created index: idx_posts_scheduled';
        END IF;
    END IF;
...
-- =================================================================
-- STEP 6: Create Basic RLS Policies (only for existing tables)
-- =================================================================
...
BEGIN
    ...
    RAISE NOTICE '  ✅ Data validation rules';
    RAISE NOTICE '  ✅ Query performance optimization';
...
-- 🎉 SUCCESS!
-- 
-- This production-ready migration:
-- 1. Checks table existence before EVERY operation
-- 2. Handles both camelCase and snake_case columns
-- 3. Skips missing tables gracefully
-- 4. Provides detailed feedback
-- 5. Won't fail on any missing objects
-- 
