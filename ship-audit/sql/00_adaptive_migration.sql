-- =================================================================
-- ADAPTIVE DATABASE MIGRATION SCRIPT FOR SUPABASE
-- Generated: 2025-08-13
-- 
-- This version automatically detects column naming (camelCase vs snake_case)
-- and adapts to your actual database schema
-- =================================================================
...
-- =================================================================
-- STEP 3: Add Soft Delete Columns (safe)
-- =================================================================
...
BEGIN
    ...
    IF v_user_id_col IS NOT NULL THEN
        EXECUTE format('CREATE INDEX IF NOT EXISTS idx_notifications_user_unread ON notifications(%I, "createdAt" DESC) WHERE read = false', v_user_id_col);
        RAISE NOTICE 'Created index: idx_notifications_user_unread on %', v_user_id_col;
    END IF;

    -- API usage index
    v_user_id_col := get_column_name('api_usage', ARRAY['userId', 'user_id']);
    IF v_user_id_col IS NOT NULL THEN
        EXECUTE format('CREATE INDEX IF NOT EXISTS idx_api_usage_user_date ON api_usage(%I, "createdAt" DESC)', v_user_id_col);
        RAISE NOTICE 'Created index: idx_api_usage_user_date on %', v_user_id_col;
    END IF;
END $$;

-- =================================================================
-- STEP 5: Enable Row Level Security (Supabase-compatible)
-- =================================================================
...
BEGIN
    ...
    IF v_user_id_col IS NOT NULL THEN
        ...
        RAISE NOTICE 'Created RLS policies for campaigns table using column %', v_user_id_col;
    END IF;
...
-- SUCCESS!
-- 
-- This adaptive migration script:
-- 1. Automatically detects column naming (camelCase vs snake_case)
-- 2. Adapts to your actual database schema
-- 3. Provides detailed feedback on what was added
-- 4. Won't fail on naming mismatches
-- 5. Is completely safe to run multiple times
-- 
-- The script has successfully adapted your database for:
-- - Better performance
-- - Enhanced security
-- - Data integrity
-- - Audit capabilities
