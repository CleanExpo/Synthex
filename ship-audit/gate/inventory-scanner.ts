...
class InventoryScanner {
  ...
  private async scanRoutes() {
    ...
    const pagesFiles = await glob('pages/**/*.{tsx,ts,jsx,js}', { cwd: this.rootDir });
  ...
  private async scanAPICalls() {
    ...
    console.log(`  Found ${this.apiCalls.length} API calls`);
  }

  private extractMethod(line: string): string | null {
    const methodMatch = line.match(/method:\s*['"`](GET|POST|PUT|DELETE|PATCH)['"`]/);
    return methodMatch ? methodMatch[1] : null;
  }

  private async scanEnvUsage() {
