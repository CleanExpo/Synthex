...
export function DroppableTimeSlot({
  ...
  const isCurrentHour = () => {
    const now = new Date();
    return (
      now.getHours() === hour && now.toDateString() === date.toDateString()
    );
  };
  ...
  return (
    <div
      ...
      className={`
        ...
        ${isOver ? 'bg-amber-500/20 ring-2 ring-amber-500 ring-inset' : ''}
        ${isOptimalForDrag && active ? 'bg-green-500/10' : ''}
        ${isCurrentHour() ? 'bg-amber-500/5' : ''}
    ...
    >
      ...
      {showHourLabel && (
        <div
          ...
          ${isCurrentHour() ? 'text-amber-400 font-medium' : ''}
        `}
        >
          {formatHour(hour)}
        </div>
      ...
      >
        ...
        {isOver && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="bg-amber-500/30 backdrop-blur-sm rounded-lg px-4 py-2 text-sm text-amber-200">
      ...
      {isCurrentHour() && (
        <div
          className="absolute left-0 right-0 h-0.5 bg-amber-500 pointer-events-none"
          style={{
