...
const CONNECTOR_DESCRIPTIONS: Record<string, string> = {
  'semantic-scholar':
    'Validates scientific and statistical claims against 214M academic papers',
  'gov-au': 'Verifies Australian government statistics and policy references',
  'industry-registry':
    'Validates IICRC standards, ASIC registrations, and NCC compliance',
  'claude-web-search':
    'Searches authoritative .gov, .edu, and .org domains for claim verification',
};
      ...
      {connectors.map(connector => (
        ...
        >
          <div className="flex items-center gap-2 mb-2">
            <span
              className={`w-2 h-2 rounded-full ${connector.enabled ? 'bg-emerald-400' : 'bg-slate-600'}`}
            />
          ...
          <p
            className={`text-xs mt-1 font-medium ${connector.enabled ? 'text-emerald-400' : 'text-slate-500'}`}
          >
