...
export function AllBusinessesDashboard() {
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[...Array(3)].map((_, i) => (
            <div
              key={i}
              className="h-48 rounded-xl bg-white/[0.04] animate-pulse"
            />
          ))}
        </div>
      </div>
  ...
  const totalCampaigns = businesses.reduce(
    (s, b) => s + (b.stats?.totalCampaigns ?? 0),
    0
  );
  const totalPosts = businesses.reduce(
    (s, b) => s + (b.stats?.totalPosts ?? 0),
    0
  );
  const totalEngagement = businesses.reduce(
    (s, b) => s + (b.stats?.totalEngagement ?? 0),
    0
  );
  const activePlatforms = businesses.reduce(
    (s, b) => s + (b.stats?.activePlatforms ?? 0),
    0
  );
              ...
              <div className="flex items-start justify-between gap-2">
                ...
                <Badge
                  ...
                  className={cn(
                    ...
                    billingBadgeClass(business.billingStatus)
                  )}
                >
