...
export function BillingTab({
  ...
  const PLAN_META: Record<string, { label: string; price: string; colour: string; borderColour: string }> = {
    starter:    { label: 'Starter',  price: '$99 AUD/mo',  colour: 'text-cyan-300',    borderColour: 'border-cyan-500/20' },
    pro:        { label: 'Pro',      price: '$249 AUD/mo', colour: 'text-cyan-300',    borderColour: 'border-cyan-500/20' },
    growth:     { label: 'Growth',   price: '$449 AUD/mo', colour: 'text-emerald-300', borderColour: 'border-emerald-500/20' },
    scale:      { label: 'Scale',    price: '$799 AUD/mo', colour: 'text-amber-300',   borderColour: 'border-amber-500/20' },
    enterprise: { label: 'Enterprise', price: 'Custom',   colour: 'text-white/80',    borderColour: 'border-white/[0.1]' },
  };
          ...
          {isFreePlan ? (
            ...
            <div className="p-5 border-[0.5px] border-white/[0.06] bg-white/[0.02] rounded-sm">
              <div className="flex items-center justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-1.5">
                    ...
                    <h3 className="text-base font-light text-white">Free Plan</h3>
                    <Badge className="bg-cyan-500/10 text-cyan-400 border-[0.5px] border-cyan-500/20 text-[10px]">
                      Active
                    </Badge>
                  </div>
              ...
              <div className="mt-4 pt-4 border-t border-white/[0.06]">
                <p className="text-[10px] uppercase tracking-[0.2em] text-white/25 mb-3">Included on free</p>
                <div className="flex flex-wrap gap-x-6 gap-y-2">
                  {['2 social accounts', '10 AI posts/mo', '1 persona', '3 GEO analyses'].map(f => (
            ...
            <div className="p-5 border-[0.5px] border-cyan-500/20 bg-cyan-500/[0.03] rounded-sm">
              <div className="flex items-center justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-1.5">
                    <Sparkles className="w-4 h-4 text-cyan-400" />
                    <h3 className="text-base font-light text-white">Starter Plan</h3>
                    <Badge className="bg-cyan-500/10 text-cyan-400 border-[0.5px] border-cyan-500/20 text-[10px]">
                      Active
                    </Badge>
                  </div>
                  <p className="text-xs text-white/40">
                    {billing.price ?? '$99 AUD'}/{billing.billingCycle ?? 'mo'} • Next billing {billing.nextBilling}
                  </p>
                </div>
                <Button onClick={onUpgrade} variant="glass-primary" size="sm">
                  <Zap className="w-3 h-3 mr-1.5" />
              ...
              <div className="mt-4 pt-4 border-t border-white/[0.06]">
                <p className="text-[10px] uppercase tracking-[0.2em] text-white/25 mb-3">Starter includes</p>
                <div className="flex flex-wrap gap-x-6 gap-y-2">
                  {[
                    '3 social accounts',
                    '50 AI posts/mo',
            ...
            <div className={`flex items-center justify-between p-5 border-[0.5px] ${planMeta?.borderColour ?? 'border-white/[0.08]'} bg-white/[0.02] rounded-sm`}>
              <div>
                <div className="flex items-center gap-2 mb-1.5">
                  <Sparkles className={`w-4 h-4 ${planMeta?.colour ?? 'text-cyan-400'}`} />
                  <h3 className={`text-base font-light ${planMeta?.colour ?? 'text-white'}`}>
                    {planMeta?.label ?? billing.plan} Plan
                  </h3>
                </div>
                <p className="text-xs text-white/40">
                  {billing.price}/{billing.billingCycle} • Next billing {billing.nextBilling}
                </p>
              </div>
              <Button onClick={onUpgrade} variant="glass-primary" size="sm">
                Upgrade Plan
        ...
        <Card variant="glass">
          ...
          <CardContent>
            <div className="flex items-center gap-4 p-4 border-[0.5px] border-white/[0.06] bg-white/[0.02] rounded-sm">
              <CreditCard className="w-6 h-6 text-cyan-400/60" />
              <div>
                <p className="text-sm text-white">{billing.paymentMethod} •••• {billing.cardLast4}</p>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-white/40 mt-0.5">{invoice.date}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                    <span className="text-sm text-white/70 font-mono tabular-nums">{invoice.amount}</span>
                    <Badge
                      className={
                        invoice.status === 'paid'
                          ? 'bg-emerald-500/10 text-emerald-400 border-[0.5px] border-emerald-500/20 text-[10px]'
                          : 'bg-amber-500/10 text-amber-400 border-[0.5px] border-amber-500/20 text-[10px]'
                      }
                    >
