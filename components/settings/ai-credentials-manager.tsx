...
import { AlertCircle, Check, Key, Plus, Trash2 } from '@/components/icons';
  ...
  const handleAddCredential = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        toast.success(
          `${getProviderName(selectedProvider)} key saved and validated!`
        );
      ...
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          ...
          <CardDescription>
            ...
            <strong className="text-orange-400">OpenRouter is required</strong>{' '}
                ...
                {PROVIDERS.map(provider => (
                  ...
                  >
                    <p className={`text-sm font-medium ${provider.color}`}>
                      ...
                      {provider.required &&
                        !existingProviders.has(provider.id) && (
                          <span className="ml-2 text-[10px] px-1.5 py-0.5 rounded bg-orange-500/20 text-orange-300 uppercase tracking-wide">
                            Required
                          </span>
                        )}
                      {existingProviders.has(provider.id) && (
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    Your key will be validated with{' '}
                    {getProviderName(selectedProvider)} before saving.
                  </p>
                </div>
                  ...
                  <div>
                    ...
                    >
                      ...
                      <optgroup label="Budget-friendly">
                        {OPENROUTER_MODELS.filter(m => m.tier === 'budget').map(
                          m => (
                            <option key={m.id} value={m.id}>
                              {m.name}
                            </option>
                    ...
                    <p className="text-xs text-slate-500 mt-1">
                      Choose which model Synthex uses for content generation.
        ...
        {isLoading ? (
          ...
          <div className="text-center py-8">
            ...
            <p className="text-slate-500 text-xs mt-1 max-w-sm mx-auto">
              Add your <strong className="text-orange-400">OpenRouter</strong>{' '}
