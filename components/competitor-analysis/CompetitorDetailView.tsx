              ...
              <div className="grid gap-3">
                {Object.entries(competitor.metrics.followers)
                  .filter(
                    ([key]) =>
                      key !== 'total' &&
                      competitor.metrics.followers[key as keyof PlatformMetrics]
                  )
                  .map(([platform, followers]) => {
                    const Icon = getPlatformIcon(platform);
                    const engagement =
                      competitor.metrics.engagement[
                        platform as keyof PlatformMetrics
                      ] || 0;
                    return (
                      <div
                        key={platform}
            ...
            <div>
              ...
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                      data={competitor.contentStrategy.contentTypes}
                      dataKey="percentage"
                      nameKey="type"
                      cx="50%"
                      cy="50%"
                      ...
                      label
                    >
                      {competitor.contentStrategy.contentTypes.map(
                        (_, index) => (
                          <Cell
                            key={index}
                            fill={
              ...
              <div className="space-y-3">
                <div className="p-3 bg-black/20 rounded">
                  <p className="text-sm text-gray-300">
                    ...
                    {competitor.name} has strong engagement on Instagram (
                    {competitor.metrics.engagement.instagram}%) but relatively
                    low posting frequency. Increasing posts could drive
                    significant growth.
                  </p>
                </div>
                <div className="p-3 bg-black/20 rounded">
                  <p className="text-sm text-gray-300">
                    <strong className="text-white">Content Gap:</strong> Only
                    20% user-generated content despite high engagement rates.
                    Encouraging more UGC could boost authenticity.
                  </p>
                </div>
                <div className="p-3 bg-black/20 rounded">
                  <p className="text-sm text-gray-300">
                    <strong className="text-white">Platform Strategy:</strong>{' '}
                    Consider their TikTok absence as an opportunity to capture
                    younger audience before they establish presence.
                  </p>
                </div>
              </div>
            </div>
