  ...
  (
    ...
    const expandSpecificTargetedElements = useCallback(
      (elements?: TreeViewElement[], selectId?: string) => {
        ...
        const findParent = (current: TreeViewElement, currentPath: string[] = []) => {
  ...
  ({ className, ...props }, ref) => {
    ...
    return (
      <div
        ...
        ref={ref}
        className={cn(
          'h-full w-px bg-white/[0.06] absolute left-1.5 rtl:right-1.5 py-3 rounded-md hover:bg-cyan-500/20 duration-300 ease-in-out',
          className,
        )}
        {...props}
      />
      ...
      >
        <AccordionPrimitive.Trigger
          className={cn(
            ...
            isSelect && isSelectable && 'bg-cyan-500/[0.06]',
            isSelectable ? 'cursor-pointer text-white/60 hover:text-white' : 'cursor-not-allowed opacity-50 text-white/30',
            className,
          )}
        ...
        >
          {expandedItems?.includes(value as string)
            ? openIcon ?? <FolderOpenIcon className="size-4 text-cyan-400/70 shrink-0" />
            : closeIcon ?? <FolderIcon className="size-4 text-white/40 shrink-0" />}
          <span>{element}</span>
        </AccordionPrimitive.Trigger>
      ...
      <AccordionPrimitive.Item value={value} className="relative">
        <AccordionPrimitive.Trigger
          ...
          className={cn(
            ...
            isSelected && isSelectable && 'bg-cyan-500/[0.06] text-white',
            !isSelected && isSelectable && 'text-white/60 hover:text-white hover:bg-white/[0.02]',
...
>(({ className, elements, expandAll = false, children, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      onClick={expandedItems && expandedItems.length > 0 ? closeAll : () => expandAllTree(elements)}
      ref={ref}
      {...props}
    >
      {children}
