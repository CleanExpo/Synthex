    ...
    >
      <div className="mx-auto max-w-content px-5">
        <div className="mb-14 max-w-2xl">
          ...
          >
            From rough idea to measured outcome — step by step
          </h2>
          <p className="mt-5 text-base leading-8 text-sx-text-secondary">
            Marketing workflow automation with editorial workflow software
            discipline: every handoff visible, every claim evidence-backed.
          </p>
        </div>
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_1.2fr] lg:items-start">
          <ol className="space-y-0">
            {walkthroughSteps.map((step, index) => (
              <li
                key={step.title}
                className="relative border-l border-white/[0.08] py-5 pl-8 first:pt-0 last:pb-0"
              >
                <span
                  className="absolute -left-[5px] top-6 flex h-2.5 w-2.5 rounded-full bg-sx-accent"
                  aria-hidden
                />
          ...
          <div className="sticky top-28 overflow-hidden rounded-card border border-white/[0.08] bg-sx-bg-primary p-3 shadow-[var(--sx-shadow-elevated)]">
            <video
