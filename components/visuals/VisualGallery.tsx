...
import { Image as ImageIcon, RefreshCw, Download } from '@/components/icons';
...
export function VisualGallery({ initialAssets }: VisualGalleryProps) {
  ...
  const fetchAssets = async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/visuals?limit=20', {
        credentials: 'include',
      });
      if (res.ok) {
        ...
        {loading ? (
          ...
          <div className="text-center py-12 text-gray-300">
            ...
            <p className="text-xs mt-1">
              Use Paper Banana to create diagrams and charts
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {assets.map(asset => (
              <div
                key={asset.id}
                className="group relative rounded-lg overflow-hidden border border-white/[0.05] bg-white/[0.02]"
              >
