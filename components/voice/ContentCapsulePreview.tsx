...
function extractabilityLabel(score: number): {
  label: string;
  colour: string;
  ring: string;
} {
...
export function ContentCapsulePreview({
  result,
  originalText,
  className,
}: ContentCapsulePreviewProps) {
  ...
  const {
    ...
    colour: scoreColour,
    ring: scoreRing,
  } = extractabilityLabel(result.extractability);
    ...
    >
      ...
      <div>
        ...
        <p className="text-xs text-slate-500 mt-0.5">
          {result.wordCount.toLocaleString()} words · Created{' '}
      ...
      <div className="flex items-centre gap-4">
        ...
        >
          <span className={cn('text-xl font-bold leading-none', scoreColour)}>
            {scorePct}
          </span>
        ...
        <div>
          <p className={cn('text-sm font-semibold', scoreColour)}>
            {scoreLabel} extractability
          </p>
          <p className="text-xs text-slate-500 mt-0.5">
