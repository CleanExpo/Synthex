...
export function BrandMentionsFeed({ brandId }: BrandMentionsFeedProps) {
  ...
  const { mutate: triggerPoll, isLoading: polling } = useMutation<
    { newCount: number; totalFetched: number },
    { brandId: string }
  >(async vars => {
      ...
      {data && data.mentions.length === 0 && (
        <div className="bg-white/[0.03] border border-white/10 rounded-xl p-8 text-center">
          ...
          <p className="text-xs text-gray-500 mt-1">
            Click "Poll for new mentions" to search for brand mentions across
            ...
            return (
              ...
              >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {mention.description && (
                      <p className="text-xs text-gray-300 mt-1 line-clamp-2">
                        {mention.description.slice(0, 120)}
                        {mention.description.length > 120 ? '…' : ''}
                      </p>
                    )}
                  </div>
                  <span
                ...
                <div className="flex items-center gap-3 text-xs text-gray-500">
                  ...
                  <span>
                    {formatRelativeDate(
                      mention.publishedAt ?? mention.createdAt
                    )}
                  </span>
                  <span>·</span>
                  <span className="bg-white/[0.05] px-2 py-0.5 rounded-full">
                    {apiSourceLabel(mention.apiSource)}
                  </span>
                </div>
              </div>
            );
