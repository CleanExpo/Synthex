...
): Array<{ weekStart: string; events: CalendarEvent[] }> {
...
export function BrandCalendarView({ brandId }: BrandCalendarViewProps) {
  ...
  const { mutate: generateCalendar, isLoading: loading } = useMutation<
    BrandCalendar,
    { brandId: string }
  >(async vars => {
      ...
      {!calendar && !loading && (
        <div className="bg-white/[0.03] border border-white/10 rounded-xl p-8 text-center">
          ...
          <p className="text-xs text-gray-500 mt-1">
            Generate a 90-day brand publishing and maintenance schedule with
            publishing, social, and credential events.
          </p>
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              ...
              {
                label: 'Maintenance',
                value: calendar.summary.maintenanceEvents,
              },
            ].map(chip => (
              <div
                key={chip.label}
          ...
          <div className="flex flex-wrap gap-2">
            {(
              Object.entries(EVENT_TYPE_META) as Array<
                [CalendarEventType, (typeof EVENT_TYPE_META)[CalendarEventType]]
              >
            ).map(([type, meta]) => (
              <span
                key={type}
                      ...
                      >
                        <div
                          className={cn(
                            ...
                            PRIORITY_INDICATOR[event.priority] ?? 'bg-gray-500'
                          )}
                        />
          ...
          <p className="text-xs text-gray-500 flex items-center gap-1.5">
            ...
            Calendar covers {calendar.coverageDays} days · Generated{' '}
            {new Date(calendar.generatedAt).toLocaleDateString('en-AU')}
          </p>
        </div>
      )}
    </div>
