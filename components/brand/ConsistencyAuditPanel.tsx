...
function matchBadge(match: NameMatchType): {
  label: string;
  className: string;
} {
  switch (match) {
...
export function ConsistencyAuditPanel({
  report,
  loading,
  onRunAudit,
}: ConsistencyAuditPanelProps) {
  return (
          ...
          >
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">
                Weighted average across {report.results.length} platform
            ...
            <span
              className={cn(
                ...
                scoreGradeClass(report.overallScore)
              )}
            >
              {report.overallScore}
            </span>
            ...
            <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4 space-y-2">
              ...
              <ul className="space-y-1">
                {report.criticalIssues.map((issue, i) => (
                  <li
                    key={i}
                    className="text-xs text-red-200 flex items-start gap-2"
                  >
