...
export function ShareDialog({
  ...
  const { shares, shareContent, revokeShare, isLoading, getShareUrl } =
    useContentShare({
      contentType,
      contentId,
      autoLoad: isOpen,
      ...
      <div className="relative w-full max-w-lg mx-4 bg-gray-950 border border-white/10 rounded-xl shadow-2xl">
        ...
        <div className="flex items-center justify-between p-4 border-b border-white/10">
          <div>
            ...
            {contentTitle && (
              <p className="text-sm text-white/60 truncate max-w-[300px]">
                {contentTitle}
              </p>
            )}
          </div>
          ...
          {activeTab === 'people' ? (
            ...
            <>
              ...
              <div className="space-y-4">
                ...
                {shares.filter(
                  s => s.accessLink && !s.sharedWithUserId && !s.sharedWithEmail
                ).length > 0 && (
                  <div className="space-y-2">
                    <h3 className="text-sm font-medium text-white/60">
                      Active Links
