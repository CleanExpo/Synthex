...
export function InsightsWidget({ className }: { className?: string }) {
  ...
  const pendingReview =
    data?.insights?.flatMap(r => r.stepExecutions).slice(0, 3) ?? [];
  const lastRun = data?.lastRun ?? null;

  return (
    <div
      className={cn(
        'rounded-xl border border-white/10 bg-white/5 p-5 space-y-4',
        className
      )}
    >
      {/* Header */}
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            <div className="rounded-lg bg-white/5 border border-white/10 p-2 text-center">
              <div className="text-base font-bold tabular-nums text-emerald-400">
                {latestRun.outputData.autoDrafted ??
                  latestRun.outputData.drafts ??
                  0}
              </div>
