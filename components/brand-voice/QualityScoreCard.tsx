...
function ScoreGauge({ value }: { value: number }) {
  ...
  const bgColour =
    pct >= 85 ? 'bg-emerald-500' : pct >= 70 ? 'bg-orange-500' : 'bg-rose-500';
...
export function QualityScoreCard({
      ...
      <div className={cn('flex items-center gap-3', className)}>
        ...
        {score.flags.length > 0 && (
          <span className="text-xs text-white/40">
            {score.flags.length} flag{score.flags.length !== 1 ? 's' : ''}
          </span>
        )}
      </div>
    );
  ...
  return (
    <div
      className={cn(
        'rounded-xl border border-white/10 bg-white/5 p-5 space-y-5',
        className
      )}
    >
      {/* Header */}
      ...
      <div className="space-y-3">
        ...
        <DimensionBar
          ...
          value={score.dimensions.brandAlignment}
        />
        ...
        <DimensionBar
          ...
          value={score.dimensions.appropriateness}
        />
      </div>
        ...
        <div className="space-y-2">
          ...
          <ul className="space-y-1">
            {score.flags.map((flag, i) => (
              <li
                key={i}
                className="flex items-start gap-2 text-xs text-orange-300/80"
              >
