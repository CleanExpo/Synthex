  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        ...
        />
        {unreadCount > 0 && (
          <span className="absolute -top-1 -right-1 h-4 w-4 rounded-full bg-amber-500 text-[10px] font-medium text-white flex items-center justify-center animate-in zoom-in duration-300">
            {unreadCount}
          </span>
        )}
      </Button>
        ...
        <div className="absolute right-0 top-12 w-80 max-h-96 overflow-hidden rounded-lg bg-white/[0.02] backdrop-blur-xl border border-white/[0.08] shadow-2xl animate-in slide-in-from-top-2 duration-300">
          ...
          {notifications.length > 0 && (
            <div className="p-3 border-t border-white/10 flex gap-2">
              ...
              <Button
                variant="ghost"
                className="flex-1 text-xs text-amber-400 hover:text-amber-300"
                onClick={() => {
                  setIsOpen(false);
                  setCentreOpen(true);
                }}
