...
function NetworkCard({
  ...
  const color =
    NETWORK_COLORS[network.slug as NetworkSlug] || NETWORK_COLORS.custom;
      ...
      <div className="flex items-start justify-between mb-4">
        ...
        <div className="flex items-center gap-1">
          ...
          <span
            className={cn(
              'text-xs font-medium',
              network.isActive ? 'text-emerald-400' : 'text-white/50'
            )}
          >
...
export function NetworkList({
  ...
  if (networks.length === 0) {
    return (
      ...
      >
        <Globe className="h-10 w-10 text-white/50 mx-auto mb-3" />
        <h3 className="text-lg font-semibold text-white mb-1">
          No networks configured
        </h3>
        <p className="text-white/50 text-sm">
          Add an affiliate network to start tracking links.
        </p>
      </div>
    );
  }
