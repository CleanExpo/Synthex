...
export function FirstWeekWidget() {
  ...
  if (!data?.hasKickstart || data.totalCount === 0) {
    ...
    return (
      <div className="border-[0.5px] border-cyan-500/20 bg-cyan-500/[0.02] rounded-sm p-5">
        <div className="flex items-center gap-3 mb-4">
          <div className="h-9 w-9 border-[0.5px] border-cyan-500/30 bg-cyan-500/[0.08] rounded-sm flex items-center justify-center flex-shrink-0">
        ...
        <button
          ...
          disabled={generating}
          className="w-full flex items-center justify-center gap-2 py-2.5 text-xs font-medium tracking-wide rounded-sm transition-colors bg-cyan-500/[0.08] hover:bg-cyan-500/[0.15] text-cyan-300 border-[0.5px] border-cyan-500/20 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {generating ? (
            <>
  ...
  return (
    <div className="border-[0.5px] border-cyan-500/20 bg-cyan-500/[0.02] rounded-sm overflow-hidden">
      {/* Header */}
      <div className="px-5 py-4 flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-light text-white tracking-tight">Your First Week Content</h3>
            <p className="text-xs text-white/40 mt-0.5">
        ...
        <Link
          ...
          className="flex items-center gap-1 text-[10px] text-cyan-400 hover:text-cyan-300 transition-colors flex-shrink-0"
        >
          View all
      ...
      <div className="border-t-[0.5px] border-white/[0.06] grid grid-cols-3 divide-x-[0.5px] divide-white/[0.06]">
        <div className="px-4 py-3 text-center">
          <p className="font-mono text-xl font-medium text-white tabular-nums">{totalCount}</p>
          <p className="text-[9px] uppercase tracking-[0.2em] text-white/25 mt-0.5">Total</p>
        </div>
        <div className="px-4 py-3 text-center">
          <p className="font-mono text-xl font-medium text-amber-400 tabular-nums">{draftsCount}</p>
          <p className="text-[9px] uppercase tracking-[0.2em] text-white/25 mt-0.5">Drafts</p>
        </div>
        <div className="px-4 py-3 text-center">
          <p className="font-mono text-xl font-medium text-emerald-400 tabular-nums">{scheduledCount}</p>
          <p className="text-[9px] uppercase tracking-[0.2em] text-white/25 mt-0.5">Scheduled</p>
        </div>
      </div>
      ...
      <div className="border-t-[0.5px] border-white/[0.06] p-5">
        <Link
          href="/dashboard/content/drafts"
          className="flex items-center justify-center gap-2 w-full py-2.5 text-xs font-medium tracking-wide rounded-sm transition-colors bg-cyan-500/[0.08] hover:bg-cyan-500/[0.15] text-cyan-300 border-[0.5px] border-cyan-500/20"
        >
