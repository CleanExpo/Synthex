...
export function SubmissionDeadlineList({
  deadlines,
  isLoading = false,
}: SubmissionDeadlineListProps) {
  if (isLoading) {
    return (
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <p className="text-slate-600 text-xs mt-1">
          Award deadlines within the next 90 days will appear here.
        </p>
      </div>
    );
  }
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-slate-300">
              {new Date(dl.deadline).toLocaleDateString('en-AU', {
                day: 'numeric',
                month: 'long',
                year: 'numeric',
              })}
          ...
          <span
            className={cn(
              ...
              PRIORITY_DOT[dl.priority]
            )}
            title={`Priority: ${dl.priority}`}
          />
