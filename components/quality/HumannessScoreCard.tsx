...
function ScoreCircle({ score, grade }: { score: number; grade: string }) {
  return (
    ...
    >
      <span
        className={cn('text-4xl font-bold tabular-nums', gradeColour(grade))}
      >
        {Math.round(score)}
      </span>
...
export function HumannessScoreCard({
  result,
  className,
}: HumannessScoreCardProps) {
  const {
    score,
    grade,
    passes,
  ...
  return (
    <div
      className={cn(
        'bg-white/5 border border-white/10 backdrop-blur-sm rounded-xl p-5 space-y-5',
        className
      )}
    >
      ...
      <div className="border-t border-white/10 pt-4">
        ...
        <div className="flex justify-around gap-2">
          <ComponentBar
            label="Slop Penalty"
            value={components.slopPenalty}
            type="penalty"
          />
          <div className="w-px bg-white/10" />
          <ComponentBar
            label="Vocabulary Bonus"
            value={components.ttrBonus}
            type="bonus"
          />
          <ComponentBar
            label="Rhythm Bonus"
            value={components.rhythmBonus}
            type="bonus"
          />
          <ComponentBar
            label="Readability Bonus"
            value={components.readabilityBonus}
            type="bonus"
          />
        </div>
        <p className="text-xs text-slate-600 mt-2 text-center">
          100 − {components.slopPenalty} (slop) +{' '}
          {components.ttrBonus +
            components.rhythmBonus +
            components.readabilityBonus}{' '}
          (bonuses) = {Math.round(score)}
        </p>
      </div>
      ...
      {issues.length === 0 && suggestions.length === 0 && (
        <div className="border-t border-white/10 pt-4 flex items-center gap-3 bg-emerald-500/10 border-emerald-500/20 rounded-lg p-3">
          ...
          <p className="text-sm text-emerald-300">
            No issues detected. Content reads as authentically human.
          </p>
        </div>
      )}
    </div>
