...
| 3   | 🟡 **PARTIAL** (clients console slice 1 `#471`; `Contact` + `clientId` schema started on branch `claude/crm-links-003b`, pushed not merged — needs additive migration + Zod + tests) **Unify CRM primitives** — fold `Lead`/`DealDeliverable`/`ClientHealthScore`/`ClientEngagementEvent` into `Client`/`Contact` + client console | CRM (#17)                 | L   | High     | P2/P3/P1 | `code-architect`+`senior-reviewer`        | Unified client list/detail from existing tables; org-scoped CRUD + Zod                                                                        |
...
| 6   | 🟡 **IN PROGRESS** (branch `claude/influencer-ugc-006`, pushed not merged: `Creator` model + `/api/creators` route + `20260616_add_creator_ugc` migration; needs RLS + Zod + tests + UGC intake) **Influencer / UGC management** — creator records + outreach + UGC intake workflow                                                | Influencer (#19)          | L   | Med-High | P2/P3/P1 | `code-architect`                          | Creator CRM + UGC intake; org-scoped; intake feeds the content pipeline                                                                       |
