...
| 3 | 🟡 **PARTIAL** (clients console slice 1 `#471`; `Contact` + `clientId` links pending — needs additive migration) **Unify CRM primitives** — fold `Lead`/`DealDeliverable`/`ClientHealthScore`/`ClientEngagementEvent` into `Client`/`Contact` + client console | CRM (#17) | L | High | P2/P3/P1 | `code-architect`+`senior-reviewer` | Unified client list/detail from existing tables; org-scoped CRUD + Zod |
