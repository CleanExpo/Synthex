...
| 6   | 🟡 **PARTIAL** (merged `#531`: `Creator`+`UgcSubmission` models, `/api/creators` GET+POST with Zod, `20260616_add_creator_ugc` RLS migration; route tests this PR; remaining: `/api/ugc` intake route + approved-submission→content-pipeline worker) **Influencer / UGC management** — creator records + outreach + UGC intake workflow | Influencer (#19)          | L   | Med-High | P2/P3/P1 | `code-architect`                          | Creator CRM + UGC intake; org-scoped; intake feeds the content pipeline                                                                       |
