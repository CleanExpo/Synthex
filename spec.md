...
| 3   | 🟡 **PARTIAL** (clients console slice 1 `#471`; merged `#532`: `Contact` model + `clientId`/`crmClientId` columns added to schema — still needs additive migration + Zod route + tests) **Unify CRM primitives** — fold `Lead`/`DealDeliverable`/`ClientHealthScore`/`ClientEngagementEvent` into `Client`/`Contact` + client console              | CRM (#17)                 | L   | High     | P2/P3/P1 | `code-architect`+`senior-reviewer`        | Unified client list/detail from existing tables; org-scoped CRUD + Zod                                                                        |
...
| 6   | 🟡 **PARTIAL** (merged `#531`: `Creator`+`UgcSubmission` models + `/api/creators` GET+POST with Zod + RLS migration; `#536`: creators route tests; `/api/ugc` GET+POST intake + PATCH moderate this PR; remaining: approved-submission→content-pipeline worker) **Influencer / UGC management** — creator records + outreach + UGC intake workflow | Influencer (#19)          | L   | Med-High | P2/P3/P1 | `code-architect`                          | Creator CRM + UGC intake; org-scoped; intake feeds the content pipeline                                                                       |
