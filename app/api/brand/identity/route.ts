...
export async function POST(request: NextRequest) {
  try {
    ...
    const identity = await prisma.brandIdentity
      .upsert({
        where: {
        ...
        create: {
          ...
          consistencyReport:
            consistencyData as unknown as Prisma.InputJsonValue,
        },
      })
      .catch(async () => {
    ...
    return NextResponse.json(
      {
        ...
        message: 'Failed to save brand identity',
      },
      { status: 500 }
    );
  }
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      {
        error: 'Internal Server Error',
        message: 'Failed to fetch brand identities',
      },
      { status: 500 }
    );
  }
