...
const parseSchema = z.object({
  instruction: z.string().min(10, 'Instruction must be at least 10 characters').max(2000, 'Instruction must be under 2000 characters'),
})
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const subscription = await subscriptionService.getSubscription(security.context.userId)
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : 'Failed to parse instruction'
