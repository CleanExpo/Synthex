/**
 * GET /api/journey/click?client_id=&moment_id=&url=
 *
 * Journey click tracker — SYN-677
 *
 ...
 *
 *   1. Validates query params (client_id, moment_id, url required)
 *   2. Validates `url` is an absolute http/https URL (prevents open redirect)
 *   3. Updates `client_journey_events.engagement_outcome` → 'clicked'
 *      (only if current outcome is 'delivered' — doesn't downgrade 'surveyed' / 'acted')
 *   4. Returns HTTP 302 redirect to the destination URL
 *
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  // Fallback destination — dashboard root if params missing or invalid
  ...
  try {
    ...
    // Only advance to 'clicked' if not already at a deeper engagement level
    ...
    if (!current || !HIGHER_OUTCOMES.has(current)) {
