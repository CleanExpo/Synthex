...
async function createNotification(
  ...
  try {
    ...
    logger.error('[refresh-tokens] Failed to create notification', {
      userId,
      type,
      error,
    });
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  // -- Authorisation ---------------------------------------------------------
  const auth = verifyCronRequest(request, 'REFRESH_TOKENS');
  if (!auth.ok) return auth.response;

  const startTime = Date.now();
  const now = new Date();
  const refreshWindowEnd = new Date(
  ...
  for (const connection of connections) {
    ...
    try {
      ...
      if (!isPlatformSupported(platform)) {
        logger.warn('[refresh-tokens] Unsupported platform — skipping', {
          id,
          platform,
        });
        skipped++;
      ...
      if (!accessToken) {
        logger.error(
          '[refresh-tokens] Cannot decrypt access token — skipping',
          { id, platform }
        );
        skipped++;
        continue;
      ...
      if (!refreshToken) {
        logger.info('[refresh-tokens] No refresh token available — skipping', {
          id,
          platform,
        });
        skipped++;
      ...
      if (!service || !service.refreshToken) {
        logger.info(
          '[refresh-tokens] Platform service has no refreshToken() — skipping',
          { id, platform }
        );
        skipped++;
        continue;
      ...
      logger.info('[refresh-tokens] Refreshing token', {
        ...
        alreadyExpired: connection.expiresAt
      ...
      if (isPermanentFailure(errorMessage)) {
        ...
        // ----------------------------------------------------------------
        logger.warn(
          '[refresh-tokens] Permanent auth failure — disabling connection',
          {
            id,
            platform,
            error: errorMessage,
