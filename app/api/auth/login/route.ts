...
export async function POST(request: NextRequest) {
  ...
  return authStrict(request, async () => {
    try {
      ...
      // Set httpOnly auth-token cookie — mirrors signup/route.ts pattern
      // Keeps JWT out of localStorage and inaccessible to client-side JS
      ...
      const cookieOptions = {
        ...
        path: '/',
        maxAge: 60 * 60 * 24 * 7, // 7 days — matches session expiry above
        ...(isProduction && {
