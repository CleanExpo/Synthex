...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      voiceScore = await (
        prismaAny.brandVoiceScore ?? prismaAny.voiceScore ?? prismaAny.brandVoice
      )?.findFirst?.({
        where: { userId },
    ...
    const data = {
      ...
      dimensions: [
        { name: 'Professionalism', score: voiceScore?.professionalismScore ?? 0, targetScore: 80, description: 'Tone appropriate for professional contexts' },
        { name: 'Authenticity', score: voiceScore?.authenticityScore ?? 0, targetScore: 85, description: 'Genuine, relatable communication style' },
        { name: 'Clarity', score: voiceScore?.clarityScore ?? 0, targetScore: 90, description: 'Clear, easy-to-understand messaging' },
        { name: 'Consistency', score: voiceScore?.consistencyScore ?? 0, targetScore: 75, description: 'Consistent brand voice across platforms' },
      ],
      recentContent: recentContent.map((c) => ({
        id: c.id,
        title: c.title ?? 'Untitled',
        platform: c.platform ?? 'unknown',
        publishedAt: c.publishedAt ? new Date(c.publishedAt).toISOString() : new Date().toISOString(),
