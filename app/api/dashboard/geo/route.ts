...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      geoRows = await (
        prismaAny.geoAnalytics ?? prismaAny.analyticsGeo ?? prismaAny.geoData
      )?.findMany({
        where: { userId },
    ...
    const totalSessions = geoRows.reduce((sum, r) => sum + (r.sessions ?? 0), 0);
    ...
    const topRegions = geoRows.map((r) => ({
      ...
      percentOfTotal: totalSessions > 0 ? Math.round(((r.sessions ?? 0) / totalSessions) * 100) : 0,
    }));
    ...
    const data = {
      totalCountries: new Set(geoRows.map((r) => r.countryCode).filter(Boolean)).size,
      totalCities: new Set(geoRows.map((r) => r.city).filter(Boolean)).size,
      topRegions,
      continentBreakdown: Object.entries(continentMap).map(([continent, sessions]) => ({
        continent,
        sessions,
        percentOfTotal: totalSessions > 0 ? Math.round((sessions / totalSessions) * 100) : 0,
      })),
    };
...
function getContinentFromCode(code: string): string {
  const map: Record<string, string> = {
    ...
    NL: 'Europe', SE: 'Europe', NO: 'Europe', PL: 'Europe', CH: 'Europe',
    AU: 'Oceania', NZ: 'Oceania',
    CN: 'Asia', JP: 'Asia', KR: 'Asia', IN: 'Asia', SG: 'Asia', ID: 'Asia',
