...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      roiRows = await (
        prismaAny.roiMetric ?? prismaAny.roiMetrics ?? prismaAny.contentRoi
      )?.findMany({
        where: { userId },
        orderBy: { period: 'desc' },
        take: 12,
    ...
    const totalInvestment = roiRows.reduce((s, r) => s + (r.investment ?? 0), 0);
    ...
    const data = {
      ...
      overallRoi,
      metrics: [
        { label: 'Total Revenue', value: totalReturn, unit: 'currency' as const, trend: 0, icon: 'dollar' as const },
        { label: 'Total Investment', value: totalInvestment, unit: 'currency' as const, trend: 0, icon: 'chart' as const },
        { label: 'ROI', value: overallRoi, unit: 'percent' as const, trend: 0, icon: 'trend' as const },
      ],
      breakdown: Object.entries(platformMap).map(([platform, { investment, return: ret }]) => ({
        platform,
        investment,
        return: ret,
        roi: investment > 0 ? Math.round(((ret - investment) / investment) * 100) : 0,
      })),
    };
