  ...
  try {
    ...
    try {
      const prismaAny = prisma as unknown as Record<string, {
        findFirst?: (args: unknown) => Promise<{ referralCode?: string | null } | null>;
      ...
      referralCode = (userRecord as { referralCode?: string | null } | null)?.referralCode ?? '';

      const referred = await prismaAny.referral?.findMany?.({
      ...
      const payout = payoutRecord as { totalEarnings?: number; pendingAmount?: number } | null | undefined;
    ...
    const activeReferrals = referredUsers.filter((r) => r.status === 'active').length;
