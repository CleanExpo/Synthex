...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      const raw = await (prisma as unknown as Record<string, unknown> & {
        experiment?: { findMany: (args: unknown) => Promise<typeof experiments> };
      }).experiment?.findMany({
        where: { userId },
    ...
    const data = {
      ...
      experiments: experiments.map((e) => ({
        ...
        status: (e.status as 'running' | 'completed' | 'paused' | 'draft') ?? 'draft',
