...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      deals = await (
        prismaAny.sponsorDeal ?? prismaAny.sponsorship ?? prismaAny.sponsor
      )?.findMany({
        where: { userId },
    ...
    const pendingDeals = deals.filter((d) => d.status === 'pending' || d.status === 'negotiating').length;
    ...
    const data = {
      ...
      deals: deals.map((d) => ({
        ...
        status: (d.status as 'active' | 'pending' | 'completed' | 'negotiating') ?? 'pending',
