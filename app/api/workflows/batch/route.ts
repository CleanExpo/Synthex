...
const startBatchSchema = z.union([
  batchByIdsSchema,
  batchByTemplateSchema,
  batchByStepsSchema,
]);
...
export async function POST(request: NextRequest) {
  ...
  if ('workflowIds' in parsed.data) {
    ...
    if (templates.length === 0) {
      return NextResponse.json(
        { error: 'No templates found for provided IDs' },
        { status: 404 }
      );
    }
    ...
    const inputs = templates.map(t => ({
      title: t.name,
      steps:
        t.steps as unknown as import('@/lib/workflow/types').WorkflowStepDefinition[],
      workflowId: t.id,
    }));
  ...
  if ('templateId' in parsed.data) {
    ...
    const execInputs = Array.from({ length: count }, (_, i) => ({
      ...
      steps:
        template.steps as unknown as import('@/lib/workflow/types').WorkflowStepDefinition[],
      inputData: inputs?.[i] ?? {},
  ...
  if ('executions' in parsed.data) {
    ...
    return NextResponse.json(
      { batchId, executions: dbExecutions, settled },
      { status: 201 }
    );
  }
