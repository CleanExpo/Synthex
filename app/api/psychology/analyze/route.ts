...
const AnalyzeRequestSchema = z.object({
  ...
  platform: z
    ...
    .optional(),
  contentType: z
    .enum(['post', 'ad', 'email', 'landing', 'tagline', 'headline'])
    .optional(),
});
...
async function _handlePost(request: NextRequest) {
  return requireApiKey(request, async userId => {
    try {
      ...
      const { content, targetAudience, platform, contentType } =
        validation.data;
      ...
      const analysis = {
        ...
        persuasionMetrics: Object.fromEntries(
          Object.entries(rawAnalysis.persuasionMetrics).map(([k, v]) => [
            k,
            clamp(v as number),
          ])
      ...
      if (generationId) {
        await psychologyAnalyzer.storeAnalysis(
          userId,
          content,
          analysis,
          generationId
