...
export async function POST(request: NextRequest) {
  ...
  if (!supabase) {
    logger.error('refresh-journey-analytics: Supabase admin client unavailable');
  ...
  const logEvent = async (status: string, message: string, durationMs: number) => {
    ...
    } catch { /* silent — observability must not block the refresh */ }
  };
  ...
  try {
    ...
    if (error) {
      ...
      await logEvent('warning', `execute_sql RPC unavailable: ${(error as { message: string }).message}`, Date.now() - startMs);

      return NextResponse.json(
        {
          success: false,
          view: 'journey_analytics',
          warning: 'execute_sql RPC not available — run REFRESH MATERIALIZED VIEW CONCURRENTLY journey_analytics manually or schedule via pg_cron',
    ...
    logger.error('refresh-journey-analytics: unexpected error', { message, durationMs });
