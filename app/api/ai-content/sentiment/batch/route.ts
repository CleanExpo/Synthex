  ...
  items: z
    .array(
      z.object({
        ...
        contentType: z
          .enum(['post', 'comment', 'campaign', 'text'])
          .default('text'),
...
async function analyzeBatch(texts: string[]): Promise<SentimentResult[]> {
...
Return a JSON array with one result per input text. Return ONLY valid JSON array.`,
            },
            {
              ...
              content: JSON.stringify(
                texts.map((t, i) => ({ index: i, text: t.substring(0, 500) }))
              ),
            },
          ],
          temperature: 0.2,
...
function analyzeWithRules(text: string): SentimentResult {
  ...
  if (lowerText.match(/happy|joy|excited/))
    emotions.push({ emotion: 'joy', intensity: 0.7 });
  if (lowerText.match(/sad|disappointed/))
    emotions.push({ emotion: 'sadness', intensity: 0.7 });
  if (lowerText.match(/angry|frustrated/))
  ...
  return requireApiKey(
    ...
    async () => {
      try {
        ...
        await (prisma as unknown as PrismaWithSentiment).sentimentAnalysis
          ?.createMany({
        ...
        if (commentUpdates.length > 0) {
          for (const update of commentUpdates) {
            await (prisma as unknown as PrismaWithSentiment).contentComment
              ?.update({
                where: { id: update.id },
                data: {
