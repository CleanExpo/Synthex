...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (searchParams.get('summary') === 'true') {
      ...
      const summary = await (await getAnomalyDetector()).getAnomalySummary(userId, days);
    ...
    if (searchParams.get('detect') === 'true') {
      ...
      const result = await (await getAnomalyDetector()).detectAnomalies(userId, {
        metrics,
        platform,
        accountId,
      });
    ...
    const { anomalies, total } = await (await getAnomalyDetector()).getAnomalies(userId, {
      severity,
      metricTypes,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (action === 'config') {
      ...
      const success = await (await getAnomalyDetector()).updateConfig(
        userId,
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    if (action === 'acknowledge') {
      ...
      const success = await (await getAnomalyDetector()).acknowledgeAnomaly(
        validated.anomalyId,
        userId,
        validated.notes
      );
    ...
    if (action === 'resolve') {
      ...
      const success = await (await getAnomalyDetector()).resolveAnomaly(
        validated.anomalyId,
        userId,
