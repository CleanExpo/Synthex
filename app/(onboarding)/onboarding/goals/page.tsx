...
/**
 * Onboarding — Goals Questionnaire → 90-Day Marketing Plan (SYN-23)
 *
 ...
 *
 * Reached after the Connect step fires the AI kickstart. Results are cached to
 * sessionStorage to survive page refreshes. Continue / Skip → /dashboard.
 */
...
import type {
  ...
  ContentPillar,
} from '@/lib/onboarding/generate-plan';
...
function PlanResults({ plan }: { plan: MarketingPlan }) {
  return (
    <div className="space-y-5">
      {/* Headline + summary */}
      <div className="p-5 rounded-xl border border-orange-500/20 bg-gradient-to-br from-orange-500/5 to-transparent">
        <div className="flex items-start gap-3">
        ...
        <div className="p-4 rounded-xl border border-white/8 bg-white/3 space-y-2">
          {plan.thirtyDayActions.map((action, idx) => (
            <div key={idx} className="flex items-start gap-3">
              <span className="w-5 h-5 rounded-full bg-orange-500/15 border border-orange-500/30 text-orange-400 text-xs font-semibold flex items-center justify-center shrink-0 mt-0.5">
                {idx + 1}
              </span>
