...
function AddAwardForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!name || !organizer || !category) {
      setError('Name, organiser, and category are required.');
      return;
    }
    setSaving(true);
    setError('');
  ...
  const inputCls =
    'w-full bg-black/20 border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder-slate-600 focus:outline-none focus:ring-1 focus:ring-orange-500/50';
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
        ...
        <input
          className={inputCls}
          placeholder="Entry fee (e.g. Free)"
          value={entryFee}
      ...
      <div className="flex gap-2">
        <button
          type="submit"
          disabled={saving}
          className="px-4 py-2 bg-orange-600 hover:bg-orange-700 disabled:opacity-50 text-white text-sm rounded-lg font-medium transition-colors"
        >
    ...
    try {
      const res = await fetch('/api/directories', {
        ...
        body: JSON.stringify({
          ...
          isFree,
          isAiIndexed: isAiIdx,
          domainAuthority: da ? parseInt(da, 10) : undefined,
        }),
      });
...
function AwardsPageContent() {
  ...
  const activeTab: AwardsTab = VALID_TABS.includes(rawTab as AwardsTab)
    ? (rawTab as AwardsTab)
    : 'awards';
  ...
  }>('/api/awards', fetchJson, { revalidateOnFocus: false });

  const { data: dirsData, mutate: mutateDirs } = useSWR<{
    directories: Array<{
      id: string;
  ...
  }>('/api/directories', fetchJson, { revalidateOnFocus: false });

  const { data: summaryData, isLoading: summaryLoading } = useSWR<{
    summary: SubmissionSummary;
  }>('/api/submissions', fetchJson, { revalidateOnFocus: false });

  // ── UI state ────────────────────────────────────────────────────────────────

  const [showAwardForm, setShowAwardForm] = useState(false);
  const [showDirForm, setShowDirForm] = useState(false);
  const [awardFormPrefill, setAwardFormPrefill] = useState<
    Partial<AwardTemplate> | undefined
  >();
  const [dirFormPrefill, setDirFormPrefill] = useState<
    Partial<DirectoryTemplate> | undefined
  >();
  const [nominationAwardId, setNominationAwardId] = useState<string | null>(
    null
  );
  ...
  const nominationAward = nominationAwardId
    ? awards.find(a => a.id === nominationAwardId)
    : null;
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-2">
            <h2 className="text-sm font-medium text-slate-300">
              Nomination — {nominationAward.name}
            </h2>
            <button
              onClick={() => setNominationAwardId(null)}
              className="text-slate-500 hover:text-slate-300"
            >
              <X className="h-4 w-4" />
            </button>
          ...
          <NominationEditor
            ...
            nominationDraft={
              nominationAward.nominationDraft ??
              '(No nomination yet — click Regenerate to generate one with AI)'
            }
        ...
        <TabsContent value="awards">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-sm px-3 py-1.5 bg-orange-600 hover:bg-orange-700 text-white rounded-lg font-medium transition-colors"
            >
          ...
          {awards.length === 0 && !showAwardForm ? (
            <div className="text-center py-16">
              ...
              <p className="text-slate-600 text-xs mt-1">
                Add an award manually or browse the Templates tab to get
                started.
              </p>
            </div>
          ) : (
        ...
        <TabsContent value="directories">
          ...
          {directories.length === 0 && !showDirForm ? (
            <div className="text-center py-16">
              ...
              <p className="text-slate-600 text-xs mt-1">
                Browse the Templates tab to add high-DA directories in one
                click.
              </p>
            </div>
          ) : (
