  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <span className="text-[10px] uppercase tracking-[0.3em] text-white/25 mb-2 block">Intelligence</span>
          <h1 className="text-3xl sm:text-4xl font-extralight tracking-tight text-white">Competitive Intelligence</h1>
          <p className="mt-1.5 text-sm text-white/40 leading-relaxed">Track competitors and discover opportunities</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={stat.label} className="border-[0.5px] border-white/[0.06] bg-white/[0.01] rounded-sm p-4">
            <div className="flex items-center justify-between mb-2">
              <stat.icon className={`h-5 w-5 ${stat.color}`} />
            </div>
            <div className="text-2xl font-mono tabular-nums font-light text-white">{stat.value}</div>
      ...
      {/* Competitors Grid */}
      <div>
        <h2 className="text-sm uppercase tracking-[0.2em] text-white/40 mb-4">Tracked Competitors</h2>
        {competitors.length === 0 ? (
          <div className="border-[0.5px] border-white/[0.06] bg-white/[0.01] rounded-sm p-12 text-center">
            <Target className="h-12 w-12 text-slate-600 mx-auto mb-4" />
            <h3 className="text-base font-light text-white mb-2">No competitors tracked yet</h3>
            <p className="text-white/40 mb-4">Add your first competitor to start monitoring their social media performance.</p>
            <button
              onClick={() => setShowAddForm(true)}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {competitors.map(comp => (
              <div key={comp.id} className="border-[0.5px] border-white/[0.06] bg-white/[0.01] rounded-sm p-4 hover:bg-white/[0.03] transition-colors">
                ...
                <div className="flex flex-wrap gap-1 mb-3">
                  {Object.entries(comp.handles || {}).map(([platform, handle]) => (
                    <span key={platform} className="text-xs bg-white/[0.03] text-white/40 px-2 py-1 rounded-sm capitalize">
                      {platform}: @{handle}
                    </span>
                  ))}
                </div>
      ...
      {insights.length > 0 && (
        <div>
          <h2 className="text-sm uppercase tracking-[0.2em] text-white/40 mb-4">Strategic Insights</h2>
          <div className="space-y-3">
            {insights.slice(0, 8).map((insight, i) => (
              <div key={i} className="border-[0.5px] border-white/[0.06] bg-white/[0.01] rounded-sm p-4 flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="text-sm font-light text-white">{insight.title}</h3>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="border-[0.5px] border-white/[0.06] bg-[#080e1a] rounded-sm p-6 w-full max-w-md mx-4 shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-base font-light text-white">Add Competitor</h2>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-white/[0.03] border-[0.5px] border-white/[0.06] rounded-sm text-white text-sm placeholder:text-white/25 focus:outline-none focus:border-cyan-500/50"
                />
              </div>
              <div>
                <label className="text-xs text-white/40 mb-1 block">Twitter Handle</label>
                <input
                  type="text"
                  value={newTwitter}
              ...
              <div>
                <label className="text-xs text-white/40 mb-1 block">LinkedIn Handle</label>
                <input
                  type="text"
                  value={newLinkedin}
              ...
              <div>
                <label className="text-xs text-white/40 mb-1 block">Website (optional)</label>
                <input
                  type="url"
                  value={newWebsite}
