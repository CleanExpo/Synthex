...
const RISK_STYLES: Record<string, { badge: string; bar: string; label: string }> = {
  healthy:  { badge: 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30', bar: 'bg-emerald-500', label: 'Healthy' },
  watch:    { badge: 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30',   bar: 'bg-yellow-500', label: 'Watch' },
  at_risk:  { badge: 'bg-orange-500/20 text-orange-400 border-orange-500/30',   bar: 'bg-orange-500', label: 'At Risk' },
  critical: { badge: 'bg-red-500/20 text-red-400 border-red-500/30',            bar: 'bg-red-500',    label: 'Critical' },
};
...
function ScorePill({ score }: { score: number }) {
  const s = RISK_STYLES[
    score >= 75 ? 'healthy' : score >= 50 ? 'watch' : score >= 25 ? 'at_risk' : 'critical'
  ];
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-sm font-semibold border ${s.badge}`}>
      {score}
    </span>
  );
}
...
export default function ClientHealthPage() {
  ...
  async function triggerComputation() {
    ...
    try {
      ...
      setTriggerMsg(`Done — processed ${body.processed ?? '?'} orgs, ${body.riskAlerts ?? 0} alerts`);
      await globalMutate('/api/admin/health');
    } catch {
      setTriggerMsg('Failed to trigger computation');
    } finally {
      setTriggering(false);
    }
  }
  ...
  const atRiskCount = rows.filter(r => r.healthScore?.riskLevel === 'at_risk' || r.healthScore?.riskLevel === 'critical').length;
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-gray-400 mt-1 text-sm">
            Weekly marketing health across all active clients — sorted most at-risk first.
            {atRiskCount > 0 && (
              <span className="ml-2 text-orange-400 font-medium">{atRiskCount} need attention</span>
            )}
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="text-red-400 text-sm">Failed to load health scores.</div>
      ) : rows.length === 0 ? (
          ...
          <table className="w-full text-sm">
            <thead className="bg-white/5 border-b border-white/10">
              <tr>
                ...
                <th className="text-left px-4 py-3 text-gray-400 font-medium">Week of</th>
              </tr>
            </thead>
                ...
                return (
                  <tr key={row.id} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      ...
                      <div className="text-gray-500 text-xs capitalize">{row.plan}</div>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      {style ? (
                        <span className={`inline-flex px-2 py-0.5 rounded-full text-xs border ${style.badge}`}>
                          {style.label}
                        </span>
                      ) : (
                      ...
                      {hs ? (
                        <div className="flex items-end gap-2">
                          {DIMENSIONS.map(d => {
                            ...
                            const barColour = dimScore >= 75 ? 'bg-emerald-500'
                              : dimScore >= 50 ? 'bg-yellow-500'
                              : dimScore >= 25 ? 'bg-orange-500'
                              : 'bg-red-500';
                            return (
                              <div key={d.key} className="flex flex-col items-center gap-1" title={dim?.description ?? d.short}>
                                <ScoreBar score={dimScore} colour={dim ? barColour : 'bg-gray-700'} />
                                <span className="text-[10px] text-gray-600">{d.short}</span>
                              </div>
                            );
                          })}
                        </div>
                    ...
                    <td className="px-4 py-3 text-gray-400 text-xs">
                      {hs ? new Date(hs.weekStart).toLocaleDateString('en-AU', { day: 'numeric', month: 'short' }) : '—'}
                    </td>
                  </tr>
                );
              })}
      ...
      <div className="flex gap-4 text-xs text-gray-500">
        {Object.entries(RISK_STYLES).map(([key, s]) => (
          <span key={key} className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full border ${s.badge}`}>
            {s.label}
          </span>
        ))}
        <span className="text-gray-600">Scores computed Monday 05:00 AEDT · Internal only until validation complete</span>
      </div>
    </div>
  );
}
