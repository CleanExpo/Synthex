/**
 * Vision Board · RestoreAssist App Store launch · 2026-05-08
 *
 * SYN-923 (Vision Board UI). The Synthex-side review surface over Pi-CEO's
 * research output for SYN-915 (RestoreAssist App Store launch). Owner-only —
 * inherits the auth gate from app/dashboard/admin/layout.tsx (isOwnerEmail).
 *
 * Server component. Reads symlinked Pi-CEO artefacts at request time and
 * passes them down to client panels. Symlinks live at:
 *   .claude/external-research/pi-ceo/marketing-studio  → ~/Pi-CEO/Pi-Dev-Ops/marketing-studio
 *
 * Architecture:
 *   Pi-CEO (brain) → research files → symlink → Synthex (eyes) → Vision Board → user review
 */
...
import { AICommentaryPanel } from './panels/AICommentaryPanel';
import { EvidencePanel } from './panels/EvidencePanel';
import { ConnectionStatusPanel } from './panels/ConnectionStatusPanel';

export const dynamic = 'force-dynamic';        // research files change between reads

const T_ZERO_ISO = '2026-05-08';

export default async function RALaunchVisionBoardPage() {
    ...
    <div className="container mx-auto flex flex-col gap-6 px-6 py-8">
      ...
      <header className="flex flex-col gap-2">
        ...
        <p className="text-sm text-muted-foreground">
          Vision Board over Pi-CEO research artefacts. Brain layer (Pi-CEO/Pi-Dev-Ops) writes
          positioning, ICP, channel plan, copy, runbook. Eyes layer (this page) consolidates the
          output for review and decision.
        </p>
      </header>
      ...
      {/* Panels — 7 of them, stacked vertically; user can scroll */}
