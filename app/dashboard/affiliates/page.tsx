...
export default function AffiliatesPage() {
  ...
  const [editingNetwork, setEditingNetwork] = useState<AffiliateNetwork | null>(
    null
  );
  ...
  const handleDeleteNetwork = useCallback(
    async (network: AffiliateNetwork) => {
      if (
        !confirm(
          `Delete "${network.name}"? This will unlink all associated affiliate links.`
        )
      ) {
        return;
      }
      ...
      <div className="p-6">
        <div className="border-[0.5px] border-red-500/20 bg-red-500/[0.05] rounded-sm p-6 text-center">
          <p className="text-red-400 mb-4">
            Failed to load affiliate data: {error}
          </p>
          <button
            onClick={refetch}
              ...
              <div className="border-[0.5px] border-white/[0.06] bg-white/[0.01] rounded-sm p-5 sticky top-6">
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div className="border-[0.5px] border-white/[0.06] bg-white/[0.01] rounded-sm p-3">
                    <div className="text-lg font-mono tabular-nums font-light text-white">
