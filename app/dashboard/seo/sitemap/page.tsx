  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `Found ${data.analysis.urlCount} URLs`,
      });
    } catch (error) {
      toast({
        title: 'Analysis Failed',
        description:
          error instanceof Error ? error.message : 'Failed to analyze sitemap',
        variant: 'destructive',
      });
    } finally {
      setIsLoading(false);
            ...
            <Card className="bg-surface-base/80 backdrop-blur-xl border border-orange-500/10">
              ...
              <CardContent className="space-y-3">
                {[
                  {
                    label: 'lastmod',
                    value: result.stats.withLastmod,
                    icon: Clock,
                  },
                  {
                    label: 'changefreq',
                    value: result.stats.withChangefreq,
                    icon: Clock,
                  },
                  {
                    label: 'priority',
                    value: result.stats.withPriority,
                    icon: LinkIcon,
                  },
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-white font-medium">
                        {value}/{result.urlCount}
                      </span>
                      <span
                        className={`text-xs ${value === result.urlCount ? 'text-green-400' : 'text-yellow-400'}`}
                      >
                        (
                        {Math.round(
                          (value / Math.max(result.urlCount, 1)) * 100
                        )}
                        %)
                      </span>
                    </div>
              ...
              <CardContent>
                <div className="max-h-96 overflow-y-auto space-y-1">
                  {result.urls.slice(0, 100).map((entry, i) => (
                    <div
                      key={i}
                      className="flex items-center justify-between p-2 hover:bg-white/5 rounded text-sm"
                    >
                  ...
                  {result.urlCount > 100 && (
                    <p className="text-gray-500 text-sm text-center py-2">
                      Showing first 100 of {result.urlCount} URLs
                    </p>
                  )}
                </div>
              </CardContent>
