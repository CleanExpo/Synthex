...
function ScoreBadge({ score }: { score: number }) {
  ...
  return (
    <span
      className={`px-2 py-0.5 text-xs font-semibold rounded-full border ${color}`}
    >
      {score}/100
    </span>
  );
...
function SERPPreviewCard({
  ...
  if (previewType === 'faq') {
    const questions = Array.isArray(previewData.questions)
      ? (previewData.questions as string[])
      : [];
    const answers = Array.isArray(previewData.answers)
      ? (previewData.answers as string[])
      : [];
  ...
  if (previewType === 'product-card') {
    ...
    return (
      <div className="bg-white/5 text-white rounded-xl shadow-xl p-5 max-w-2xl">
        ...
        {googleDesc && (
          <p className="text-sm text-gray-700 mt-2">{googleDesc}</p>
        )}
      </div>
    );
  }
...
export default function SchemaMarkupManagerPage() {
  ...
  const handleUseTemplate = (fields: Record<string, unknown>) => {
    ...
    toast({
      ...
      description: 'Template loaded into editor',
    });
  };
          ...
          <div className="space-y-6">
            {!generatedSchema && !hook.richPreview ? (
              <div className="text-center py-20 text-gray-500">
                ...
                <p className="text-sm">
                  Generate or load a schema in the Editor tab first
                </p>
                <Button
                ...
                {hook.richPreview ? (
                  <div className="space-y-4">
                    ...
                    <p className="text-xs text-gray-500 italic">
                      Note: Actual search appearance may vary. Rich results
                      eligibility depends on Google's indexing and schema
                      quality.
                    </p>
                  </div>
                ) : (
