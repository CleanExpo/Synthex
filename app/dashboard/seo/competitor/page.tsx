...
export default function CompetitorPagesPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `${data.pages.length} page outlines created`,
      });
    } catch (error) {
      toast({
        title: 'Generation Failed',
        description:
          error instanceof Error ? error.message : 'Failed to generate pages',
        variant: 'destructive',
      });
    } finally {
      setIsLoading(false);
  ...
  const copySchema = async (index: number) => {
    await navigator.clipboard.writeText(
      JSON.stringify(results[index].schema, null, 2)
    );
